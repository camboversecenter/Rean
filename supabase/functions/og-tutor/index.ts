...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!tutorId || !SAFE_ID_RE.test(tutorId)) {
    ...
    const subjects =
      tutor.subjects && tutor.subjects.length > 0 ? tutor.subjects.join(', ') : 'Various Subjects';

    // 4. Escape every value that lands in the HTML below.
    const safeTitle = escapeHtml(`Tutor ${name} - ${subjects}`);
    const safeDesc = escapeHtml((tutor.bio || `Learn with ${name} on REAN.`).substring(0, 160));
    const image = escapeHtml(
      tutor.cover_image ||
        profile?.avatar_url ||
        `${supabaseUrl}/storage/v1/object/public/Rean/avatars/default-tutor.png`
    );
