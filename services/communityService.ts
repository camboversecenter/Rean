...
export const updateCommunityReply = async (replyId: string, content: string) => {
  const { error } = await supabase.from('community_replies').update({ content }).eq('id', replyId);
  if (error) throw error;
};
...
export const toggleReaction = async (
  ...
  const query = supabase.from('user_reactions').select('id, reaction').eq('user_id', user.id);
