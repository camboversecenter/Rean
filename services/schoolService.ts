...
import { School, ShortCourse, Scholarship, Admission, SchoolInquiry, CourseEnrollment } from '../types';
...
export const fetchAllSchools = async (page: number = 1, limit: number = 10, searchQuery: string = '', typeFilter: string = 'All'): Promise<School[]> => {
...
export const fetchSchoolById = async (id: string): Promise<School | null> => {
    ...
    const { data, error } = await supabase.from('schools').select(`*, admissions(*), scholarships(*), short_courses(*)`).eq('id', id).single();
    if (error) return null;
    return mapSchoolFromDB(data);
};

export const fetchCourseById = async (id: string): Promise<ShortCourse & { school: { id: string, name: string, logo: string } } | null> => {
    const { data, error } = await supabase.from('short_courses').select(`*, schools(id, name, logo)`).eq('id', id).single();
    if (error || !data) return null;
    return { ...mapCourseFromDB(data), school: { id: data.schools.id, name: data.schools.name, logo: data.schools.logo } };
};

export const fetchAllShortCourses = async (page: number = 1, limit: number = 12, category?: string, format?: string): Promise<(ShortCourse & { schoolName: string, schoolId: string })[]> => {
    const from = (page - 1) * limit;
    const to = from + limit - 1;
    // IMPORTANT: Filter by is_listed = true for public list
    let query = supabase.from('short_courses')
    ...
    const { data, error } = await query;
    if (error) return [];
    return data.map((item: any) => ({ ...mapCourseFromDB(item), schoolName: item.schools?.name, schoolId: item.schools?.id }));
};
...
export const getMySchool = async (): Promise<School | null> => {
    ...
    const { data, error } = await supabase.from('schools').select(`*, admissions(*), scholarships(*), short_courses(*)`).eq('owner_id', user.id).maybeSingle();
    if (error || !data) return null;
    return mapSchoolFromDB(data);
};
...
export const createMySchool = async (name: string): Promise<School> => {
    ...
    const { data, error } = await supabase.from('schools').insert([{ owner_id: user.id, name: name, is_published: false }]).select().single();
...
export const addAdmission = async (schoolId: string, admission: Omit<Admission, 'id' | 'scholarships'>) => {
    const { error } = await supabase.from('admissions').insert([{ school_id: schoolId, title: admission.title, description: admission.description, majors: admission.majors, start_date: admission.startDate, end_date: admission.endDate, status: admission.status }]);
    if (error) throw error;
};

export const updateAdmission = async (id: string, updates: Partial<Admission>) => {
    const dbUpdates: any = { title: updates.title, description: updates.description, majors: updates.majors, start_date: updates.startDate, end_date: updates.endDate, status: updates.status };
...
export const addScholarship = async (schoolId: string, admissionId: string | undefined, scholarship: Omit<Scholarship, 'id'>) => {
    const { error } = await supabase.from('scholarships').insert([{ school_id: schoolId, admission_id: admissionId || null, title: scholarship.title, deadline: scholarship.deadline, discount: scholarship.discount }]);
    if (error) throw error;
};
...
export const createStudentInquiry = async (inquiry: { schoolId: string; admissionId?: string; message: string; studentName: string; studentPhone: string; }) => {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error("Must be logged in");
    const { error } = await supabase.from('school_inquiries').insert([{ school_id: inquiry.schoolId, admission_id: inquiry.admissionId || null, student_id: user.id, student_name: inquiry.studentName, student_phone: inquiry.studentPhone, message: inquiry.message, status: 'Pending' }]);
    if (error) throw error;
};

export const getMyInquiries = async (schoolId: string): Promise<SchoolInquiry[]> => {
    const { data, error } = await supabase.from('school_inquiries').select(`*, admissions(title)`).eq('school_id', schoolId).order('created_at', { ascending: false });
    if (error) return [];
...
export const getSchoolEnrollments = async (schoolId: string) => {
    const { data, error } = await supabase.from('course_enrollments').select(`*, short_courses!inner(id, title, school_id)`).eq('short_courses.school_id', schoolId).order('created_at', { ascending: false });
    if (error) return [];
    return data.map(mapEnrollmentFromDB);
};

export const updateEnrollmentStatus = async (enrollmentId: string, status: string) => {
    const { error } = await supabase.from('course_enrollments').update({ status }).eq('id', enrollmentId);
    if (error) throw error;
};
...
export const enrollInCourse = async (courseId: string, studentName: string, studentPhone: string) => {
    const { data: { user } } = await supabase.auth.getUser();
