...
export const getMyTutorProfile = async (): Promise<TutorProfile | null> => {
    ...
    const { data: tutorData } = await supabase.from('tutors').select('*').eq('id', user.id).maybeSingle();
...
export const createBooking = async (booking: { tutorId: string, subject: string, scheduledTime: string, notes?: string }) => {
    const { data: { user } } = await supabase.auth.getUser();
...
export const updateBookingStatus = async (id: string, status: 'Accepted' | 'Rejected' | 'Completed') => {
    const { error } = await supabase
        .from('tutor_bookings')
        .update({ status })
        .eq('id', id);
...
export const submitHomework = async (homeworkId: string, text: string) => {
    await supabase.from('tutor_homeworks').update({
        status: 'Submitted',
...
export const fetchStudentRequests = async (page: number = 1, limit: number = 10): Promise<TutorRequest[]> => {
    const from = (page - 1) * limit;
    const to = from + limit - 1;
