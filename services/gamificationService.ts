...
export const transferBountyReward = async (recipientId: string, amount: number) => {
    if (amount <= 0) return;
    const { data: profile } = await supabase.from('profiles').select('spendable_points').eq('id', recipientId).single();
    if (profile) {
        await supabase.from('profiles').update({ spendable_points: (profile.spendable_points || 0) + amount }).eq('id', recipientId);
        await supabase.from('point_transactions').insert([{
            user_id: recipientId, amount: amount, type: 'earn', reason: 'Bounty Reward Claimed'
        }]);
        notifyPointsUpdate();
    }
};

export const fetchPointHistory = async (page: number = 1, limit: number = 20): Promise<PointTransaction[]> => {
...
export const fetchCreatorClaims = async (): Promise<RewardClaim[]> => {
    ...
    const { data: boxes } = await supabase.from('mystery_boxes').select('id, title').eq('owner_id', user.id);
...
export const getMyRewardClaims = async (): Promise<RewardClaim[]> => {
    ...
    if (boxIds.length > 0) {
        const { data: boxes } = await supabase.from('mystery_boxes').select('id, title').in('id', boxIds);
        boxMap = (boxes || []).reduce((acc: any, b: any) => { acc[b.id] = b; return acc; }, {});
    }
