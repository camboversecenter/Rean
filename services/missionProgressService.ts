...
export interface MissionEnrollment {
    ...
    progressDetails: Record<string, { status: MissionModuleStatus, submission: string, feedback: string }>; // Detailed data
...
export const addStudentByEmail = async (missionId: string, email: string, classId?: string | null) => {
    ...
    if (profileError || !profile) {
        ...
        throw new Error("Student not found. Ask them to login to REAN first so their profile is created.");
    }
...
export const updateEnrollmentStatus = async (enrollmentId: string, status: 'In Progress' | 'Rejected' | 'Completed' | 'Pending', missionId?: string) => {
    const { error } = await supabase
        .from('mission_enrollments')
        .update({ status })
        .eq('id', enrollmentId);
...
export const reviewSubmission = async (enrollmentId: string, moduleId: string, status: MissionModuleStatus, feedback: string) => {
...
export const getMissionEnrollments = async (
    ...
    const mappedData = data.map((row: any) => {
        ...
        const student = profiles.find(p => p.id === row.student_id) || { full_name: 'Unknown Student', email: '', avatar_url: '' };
...
// --- SQUAD VISIBILITY ---
export const getSquadMembers = async (missionId: string, squadId: number): Promise<SquadMember[]> => {
    const { data, error } = await supabase
        .from('mission_enrollments')
        .select('student_id')
        .eq('mission_id', missionId)
