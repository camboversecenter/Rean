    ...
    if (process.env.API_KEY) {
        ...
        try {
            ...
            if (e.message?.includes('405') || e.status === 405) {
                console.error("CRITICAL: A Service Worker is likely intercepting the API request. Please refresh the page to unregister it.");
            }
        }
    } else {
        console.warn(`Edge Function failed for ${taskName}. No local API_KEY found for fallback. Make sure API_KEY is set in your environment variables.`);
    }
    return null;
};

// --- HELPER: PARSE EVALUATION ---
const parseEvaluationResponse = (text: string): { passed: boolean; score: number; feedback: string } => {
 ...
 */
export const chatWithAiTag = async (
    message: string, 
...
export const evaluateImageSubmission = async (
    ...
    try {
        ...
        return { passed: false, score: 0, feedback: "បរាជ័យក្នុងការមើលរូបភាព។ សូមព្យាយាមម្តងទៀត ឬប្រើអក្សរជំនួស។" };
    }
};
...
export const generateMissionStructure = async (topic: string): Promise<Partial<any>> => {
    ...
    try {
        ...
        if (fallbackText) {
             const cleanText = fallbackText.replace(/```json/g, '').replace(/```/g, '').trim();
             return JSON.parse(cleanText);
        }
