...
import { 
    Building2, Camera, Zap, Brain, X, Loader2, Save, Layout, MapPin, DollarSign, BookOpen, Eye, EyeOff
} from './Icons';
...
interface SchoolFormProps {
    school: School;
    isSaving: boolean;
    onSave: (updates: Partial<School>, files: { logo: File | Blob | null, cover: File | Blob | null }) => Promise<void>;
}
...
const SchoolForm: React.FC<SchoolFormProps> = ({ school, isSaving, onSave }) => {
    ...
    const handleFormSubmit = () => {
        ...
        const majorsArray = majorsInput.split(',').map(m => m.trim()).filter(m => m !== '');
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
            ...
            <div className="lg:col-span-3">
                <div className={`p-4 rounded-2xl border-2 flex flex-col sm:flex-row justify-between items-center gap-4 transition-all ${
                    formData.isPublished 
                    ? 'bg-green-50 border-green-100' 
                    : 'bg-amber-50 border-amber-100'
                }`}>
                    <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-xl ${formData.isPublished ? 'bg-green-500 text-white' : 'bg-amber-500 text-white'}`}>
                            {formData.isPublished ? <Eye className="h-6 w-6" /> : <EyeOff className="h-6 w-6" />}
                        </div>
                        <div>
                            <h3 className="font-bold text-gray-900 flex items-center gap-2">
                                ស្ថានភាពសាលា (Visibility)
                                <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${formData.isPublished ? 'bg-green-200 text-green-800' : 'bg-amber-200 text-amber-800'}`}>
                    ...
                    <div className="flex items-center gap-3 bg-white/50 p-2 rounded-xl">
                        <span className={`text-xs font-bold ${formData.isPublished ? 'text-gray-400' : 'text-amber-600'}`}>លាក់</span>
                        <button 
                        ...
                        <span className={`text-xs font-bold ${formData.isPublished ? 'text-green-600' : 'text-gray-400'}`}>បង្ហាញ</span>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                    ...
                    <div className="space-y-5">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase">ឈ្មោះសាលា</label>
                                <input 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase">ប្រភេទគ្រឹះស្ថាន</label>
                                <select 
                                    className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none" 
                                    value={formData.type} 
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase">ការពិពណ៌នា</label>
                            <textarea 
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase">ទីតាំង</label>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase">តម្លៃសិក្សា (Tuition Range)</label>
                                <div className="relative">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase">ជំនាញសិក្សា (Majors)</label>
                            <div className="relative">
                            ...
                            <p className="text-[10px] text-gray-400 mt-1 italic">បញ្ចូលជំនាញដែលសាលាផ្តល់ជូន បំបែកដោយសញ្ញាក្បៀស (,)</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="lg:col-span-1 space-y-6">
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={() => openAiModal('cover')} className="text-[10px] font-bold text-purple-600 flex items-center bg-purple-50 px-2 py-1 rounded-lg hover:bg-purple-100 transition-colors">
                    ...
                    <div className="relative aspect-video rounded-2xl bg-gray-50 overflow-hidden border border-dashed border-gray-200 flex items-center justify-center group">
                        <img 
                            src={coverBase64 ? `data:image/png;base64,${coverBase64}` : (coverFile ? URL.createObjectURL(coverFile) : formData.coverImage)} 
                ...
                <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={() => openAiModal('logo')} className="text-[10px] font-bold text-purple-600 flex items-center bg-purple-50 px-2 py-1 rounded-lg hover:bg-purple-100 transition-colors">
                    ...
                    <div className="w-24 h-24 mx-auto relative rounded-2xl bg-gray-50 border border-dashed border-gray-200 flex items-center justify-center group overflow-hidden">
                        <img 
                            src={logoBase64 ? `data:image/png;base64,${logoBase64}` : (logoFile ? URL.createObjectURL(logoFile) : formData.logo)} 
                    ...
                    <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl animate-scale-in">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Powered by Gemini 2.5</p>
                            </div>
                        </div>
                        <p className="text-sm text-gray-600 mb-4">ពណ៌នាពីរូបភាពដែលអ្នកចង់បានសម្រាប់ <strong>{aiTarget === 'logo' ? 'ឡូហ្គោ' : 'រូបភាពគម្រប'}</strong>។</p>
                        <textarea 
                        ...
                        <div className="flex gap-2 mt-6">
                            <button onClick={() => setAiModalOpen(false)} className="flex-1 py-3 text-gray-500 font-bold hover:bg-gray-50 rounded-xl">Cancel</button>
                            <button 
                            ...
                            >
                                {generating ? <Loader2 className="h-5 w-5 animate-spin"/> : <><Zap className="h-4 w-4 mr-1"/> Generate</>}
                            </button>
                        </div>
                        <p className="text-[10px] text-center text-gray-400 mt-4 italic">ចំណាយ៖ {AI_COSTS.IMAGE} Points សម្រាប់ការបង្កើតម្តង។</p>
                    </div>
                </div>
            )}
        </div>
