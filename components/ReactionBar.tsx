...
const ReactionBar: React.FC<ReactionBarProps> = ({ reactions, userReaction, onReact, size = 'sm' }) => {
    ...
    return (
        <div className="flex items-center gap-1">
            <button 
                onClick={(e) => { e.stopPropagation(); onReact('bulb'); }}
                className={`${baseBtnClass} ${getBtnStyle('bulb')}`}
                title="មានប្រយោជន៍ (Insightful)"
            >
                <Lightbulb className={`${size === 'sm' ? 'h-3.5 w-3.5' : 'h-4 w-4'} ${userReaction === 'bulb' ? 'fill-yellow-600' : ''}`} />
                {reactions.bulb > 0 && <span>{reactions.bulb}</span>}
            </button>

            <button 
                onClick={(e) => { e.stopPropagation(); onReact('heart'); }}
                className={`${baseBtnClass} ${getBtnStyle('heart')}`}
                title="ពេញចិត្ត (Love)"
            >
                <Heart className={`${size === 'sm' ? 'h-3.5 w-3.5' : 'h-4 w-4'} ${userReaction === 'heart' ? 'fill-pink-500' : ''}`} />
                {reactions.heart > 0 && <span>{reactions.heart}</span>}
            </button>

            <button 
                onClick={(e) => { e.stopPropagation(); onReact('thumb'); }}
                className={`${baseBtnClass} ${getBtnStyle('thumb')}`}
                title="យល់ស្រប (Agree)"
            >
                <ThumbsUp className={`${size === 'sm' ? 'h-3.5 w-3.5' : 'h-4 w-4'} ${userReaction === 'thumb' ? 'fill-blue-500' : ''}`} />
