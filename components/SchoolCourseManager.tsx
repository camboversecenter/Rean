...
import { 
    BookOpen, Plus, Edit, Trash2, Loader2, CheckCircle, Zap, 
    AlertCircle, Camera, Calendar, Clock, MapPin, User, Tag, DollarSign, Users, Layout, ChevronLeft, Eye, EyeOff
} from './Icons';
import { ShortCourse } from '../types';
import { 
    addShortCourse, updateShortCourse, deleteShortCourse, 
...
const SchoolCourseManager: React.FC<SchoolCourseManagerProps> = ({ schoolId, courses, onUpdate }) => {
                                        ...
                                        <tr key={course.id} className="hover:bg-gray-50 transition-colors group">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 rounded-lg bg-gray-100 flex-shrink-0 overflow-hidden border border-gray-200">
                                                        {course.coverImage ? (
                                                            <img src={course.coverImage} className="w-full h-full object-cover" alt="Course" />
                                                        ) : (
                                                ...
                                                <div className="inline-flex flex-col items-center">
                                                    ...
                                                    <div className="w-20 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                                                        <div 
                                                            ...
                                                            style={{ width: `${Math.min(100, (course.enrolledCount || 0) / (course.maxSeats || 20) * 100)}%` }}
                                                        ></div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-center">
                                                <button 
                                                    ...
                                                    title={course.isListed ? "Visible (Click to hide)" : "Hidden (Click to show)"}
                                                >
                                                    {course.isListed ? <Eye className="h-5 w-5" /> : <EyeOff className="h-5 w-5" />}
                                                </button>
                                            </td>
        ...
        <div className="animate-fade-in bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="p-4 md:p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-50/50">
                <div className="flex items-center w-full md:w-auto">
                    <button onClick={handleCancel} className="mr-3 p-2 hover:bg-gray-200 rounded-full transition-colors flex-shrink-0">
                        <ChevronLeft className="h-5 w-5 text-gray-500" />
                    </button>
                    <h3 className="font-bold text-lg md:text-xl text-gray-900 truncate">{editingCourse?.id ? 'កែប្រែវគ្គសិក្សា' : 'បន្ថែមវគ្គសិក្សាថ្មី'}</h3>
                </div>
                
                <div className="flex flex-wrap gap-2 w-full md:w-auto justify-end">
                    {/* Editor Visibility Toggle */}
                    <button 
                        onClick={() => setEditingCourse(prev => prev ? {...prev, isListed: !prev.isListed} : null)}
                    ...
                    >
                        {editingCourse?.isListed ? <Eye className="h-4 w-4 mr-2"/> : <EyeOff className="h-4 w-4 mr-2"/>}
                        {editingCourse?.isListed ? 'Visible' : 'Hidden'}
                    </button>

                    <button onClick={handleCancel} className="flex-1 md:flex-none px-4 py-2 text-gray-500 font-bold hover:bg-gray-100 rounded-lg text-sm">បោះបង់</button>
                    <button 
                    ...
                    >
                        {saving ? <Loader2 className="h-4 w-4 animate-spin mr-2"/> : <CheckCircle className="h-4 w-4 mr-2"/>} រក្សាទុក
                    </button>
                </div>
            </div>
            ...
            <div className="p-6 space-y-8">
                    ...
                    <div className="space-y-6">
                        <div>
                            <div className="flex justify-between items-center mb-2">
                                <label className="text-xs font-bold text-gray-400 uppercase">រូបភាពគម្រប (Cover)</label>
                                <button 
                                ...
                                >
                                    {generatingImage ? <Loader2 className="h-3 w-3 animate-spin mr-1"/> : <Zap className="h-3 w-3 mr-1"/>}
                                    AI Imagine
                                </button>
                            </div>
                            <div className="relative aspect-video bg-gray-100 rounded-2xl overflow-hidden border border-dashed border-gray-300 flex items-center justify-center group cursor-pointer">
                                {coverBase64 || coverFile || editingCourse?.coverImage ? (
                                    <img 
                                        src={coverBase64 ? `data:image/png;base64,${coverBase64}` : (coverFile ? URL.createObjectURL(coverFile) : editingCourse?.coverImage)} 
                                ...
                                <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <div className="bg-white p-2 rounded-full shadow-lg"><Camera className="h-5 w-5 text-primary"/></div>
                                </div>
                                <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" accept="image/*" onChange={e => { setCoverFile(e.target.files?.[0] || null); setCoverBase64(null); }} />
                            </div>
                        </div>

                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 flex items-center"><Tag className="h-3 w-3 mr-1" /> ប្រភេទវគ្គសិក្សា</label>
                                <input 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 flex items-center"><User className="h-3 w-3 mr-1" /> គ្រូបង្រៀន (Instructor)</label>
                                <input 
                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="md:col-span-2">
                                <label className="block text-xs font-bold text-gray-500 mb-1 flex items-center"><Layout className="h-3 w-3 mr-1" /> ចំណងជើងវគ្គសិក្សា</label>
                                <input 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 flex items-center"><Calendar className="h-3 w-3 mr-1" /> ថ្ងៃចាប់ផ្តើម</label>
                                <input 
                                    type="date"
                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 flex items-center"><AlertCircle className="h-3 w-3 mr-1" /> ផុតកំណត់ទទួលពាក្យ</label>
                                <input 
                                    type="date"
                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 flex items-center"><Clock className="h-3 w-3 mr-1" /> រយៈពេល (ឧ. ៣ ខែ)</label>
                                <input 
                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 flex items-center"><MapPin className="h-3 w-3 mr-1" /> កាលវិភាគសិក្សា</label>
                                <input 
                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 flex items-center"><DollarSign className="h-3 w-3 mr-1" /> តម្លៃ (រៀល)</label>
                                <input 
                                    type="number" 
                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl font-bold focus:ring-2 focus:ring-primary/20 outline-none" 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 flex items-center"><Users className="h-3 w-3 mr-1" /> ចំនួនសិស្សអតិបរមា</label>
                                <input 
                                    type="number" 
                                    className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 outline-none" 
                                    value={editingCourse?.maxSeats || 20} 
                                    onChange={e => setEditingCourse({...editingCourse, maxSeats: parseInt(e.target.value) || 0})} 
                                />
                            </div>
                            <div className="md:col-span-2">
                                ...
                                <div className="flex gap-4">
                                    {['Online', 'On-Campus', 'Hybrid'].map(fmt => (
                                        <label key={fmt} className="flex-1 flex items-center justify-center p-3 rounded-xl border border-gray-200 bg-gray-50 cursor-pointer hover:bg-gray-100 has-[:checked]:bg-primary/5 has-[:checked]:border-primary transition-all">
                                            <input type="radio" className="hidden" name="format" checked={editingCourse?.format === fmt} onChange={() => setEditingCourse({...editingCourse, format: fmt as any})} />
                                            <span className={`text-xs font-bold ${editingCourse?.format === fmt ? 'text-primary' : 'text-gray-500'}`}>{fmt}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
                ...
                <div>
                    <label className="block text-xs font-bold text-gray-500 mb-2 flex items-center"><BookOpen className="h-3 w-3 mr-1" /> ការពិពណ៌នាវគ្គសិក្សា (Description)</label>
                    <textarea 
                ...
                <div>
                    <div className="flex justify-between items-center mb-2">
                        <label className="block text-xs font-bold text-gray-500 flex items-center"><Zap className="h-3 w-3 mr-1 text-purple-600" /> កម្មវិធីសិក្សា (Syllabus)</label>
                        <button 
                        ...
                        >
                            {generatingSyllabus ? <Loader2 className="h-3 w-3 animate-spin mr-1"/> : <Zap className="h-3 w-3 mr-1"/>}
