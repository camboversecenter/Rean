...
import { 
    GraduationCap, Plus, Calendar, Award, Edit, Trash2, X, CheckCircle, Loader2, BookOpen, FileText
} from './Icons';
...
const SchoolAdmissionManager: React.FC<SchoolAdmissionManagerProps> = ({ schoolId, admissions, onUpdate }) => {
    ...
    const [showScholarshipModal, setShowScholarshipModal] = useState<{show: boolean, admissionId?: string}>({show: false});
    const [newScholarship, setNewScholarship] = useState<Partial<Scholarship>>({ title: '', discount: '' });
    ...
    const handleOpenAdmissionModal = (adm?: Admission) => {
        if (adm) {
            ...
            setEditingAdmission({ status: 'Open', majors: [], description: '', startDate: '', endDate: '' });
            setMajorsStr('');
        }
    ...
    const handleSaveAdmission = async () => {
        ...
        setSaving(true);
        try {
            const processedMajors = majorsStr.split(',').map(m => m.trim()).filter(m => m !== '');
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {admissions.map(adm => (
                    <div key={adm.id} className="bg-white rounded-3xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                        <div className="flex justify-between items-start mb-3">
                            ...
                            <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                                adm.status === 'Open' ? 'bg-green-100 text-green-700' : 
                                adm.status === 'Closing Soon' ? 'bg-orange-100 text-orange-700' : 
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="bg-yellow-50 rounded-xl p-3 border border-yellow-100">
                                ...
                                <div className="space-y-1">
                                    {adm.scholarships.length === 0 && <p className="text-[10px] text-yellow-600 italic">គ្មានអាហារូបករណ៍</p>}
                                    {adm.scholarships.map(sch => (
                                        <div key={sch.id} className="flex justify-between items-center text-xs bg-white/50 p-1.5 rounded-lg border border-yellow-50">
                                            ...
                                            <div className="flex items-center gap-2 flex-shrink-0">
                                                ...
                                                <button onClick={() => handleDeleteScholarship(sch.id)} className="text-red-400 hover:text-red-600">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm overflow-y-auto">
                    <div className="bg-white rounded-3xl w-full max-w-lg my-8 animate-scale-in">
                        <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                            <h3 className="font-bold text-xl">{editingAdmission?.id ? 'កែប្រែការជ្រើសរើស' : 'បន្ថែមការជ្រើសរើស'}</h3>
                            <button onClick={() => setShowAdmissionModal(false)}><X className="h-6 w-6 text-gray-400"/></button>
                        </div>
                        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">ចំណងជើងយុទ្ធនាការ</label>
                                <input 
                                    ...
                                    onChange={e => setEditingAdmission({...editingAdmission, title: e.target.value})} 
                                    placeholder="ឧ. បរិញ្ញាបត្រឆ្នាំទី១ ជំនាន់ទី១៥" 
                                />
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">ការពិពណ៌នា</label>
                                <textarea 
                                    ...
                                    onChange={e => setEditingAdmission({...editingAdmission, description: e.target.value})} 
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">ថ្ងៃចាប់ផ្តើម</label>
                                    <input 
                                        type="date"
                                        className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none" 
                                        value={editingAdmission?.startDate || ''} 
                                        onChange={e => setEditingAdmission({...editingAdmission, startDate: e.target.value})} 
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">ថ្ងៃផុតកំណត់</label>
                                    <input 
                                        type="date"
                                        className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none" 
                                        value={editingAdmission?.endDate || ''} 
                                        onChange={e => setEditingAdmission({...editingAdmission, endDate: e.target.value})} 
                                    />
                                </div>
                            </div>

                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">ស្ថានភាព</label>
                                <select 
                                    className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none" 
                                    value={editingAdmission?.status || 'Open'} 
                                    onChange={e => setEditingAdmission({...editingAdmission, status: e.target.value as any})}
                                >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl w-full max-w-sm animate-scale-in">
                        <div className="p-5 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <button onClick={() => setShowScholarshipModal({show:false})}><X className="h-5 w-5 text-gray-400"/></button>
                        </div>
                        <div className="p-5 space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">ឈ្មោះអាហារូបករណ៍</label>
                                <input 
                                    className="w-full p-3 bg-gray-50 border border-gray-100 rounded-xl focus:ring-2 focus:ring-primary/20 outline-none" 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1 uppercase">ការបញ្ចុះតម្លៃ (%)</label>
                                <input 
                                    ...
                                    onChange={e => setNewScholarship({...newScholarship, discount: e.target.value})} 
                                    placeholder="ឧ. ១០០%" 
                                />
                            </div>
                            <button 
                            ...
                            >
                                {saving ? <Loader2 className="h-4 w-4 animate-spin mr-2"/> : <Award className="h-4 w-4 mr-2"/>} បញ្ជាក់
                            </button>
                        </div>
                    </div>
                </div>
