...
import { CalendarDays, Clock, Monitor, Users, MapPin, User, Building2, AlertCircle, CheckCircle, Plus } from './Icons';
...
const ShortCourseCard: React.FC<ShortCourseCardProps> = ({ course, schoolName, onClick, onCompare, isSelected }) => {
  ...
  return (
    <div className={`relative h-full flex flex-col transition-all duration-300 ${isSelected ? 'ring-2 ring-primary rounded-xl transform scale-[0.98]' : ''}`}>
        <Link 
        to={`/course/${course.id}`}
        onClick={onClick} 
            ...
            <div className="space-y-2 mt-auto">
                ...
                <div className="flex justify-between items-center pt-3 border-t border-gray-50 mt-3">
                    <span className="bg-gray-50 text-gray-600 text-[10px] font-bold px-2 py-1 rounded-md flex items-center border border-gray-100">
                        {course.format === 'Online' ? <Monitor className="h-3 w-3 mr-1" /> : <MapPin className="h-3 w-3 mr-1" />}
                        {course.format === 'Online' ? 'អនឡាញ' : (course.format === 'On-Campus' ? 'រៀននៅសាលា' : 'ចម្រុះ')}
                    </span>
                    <span className="text-sm font-bold text-primary">
                        {formatRiel(course.price)}
                    </span>
                </div>
            </div>
        </div>
