...
import { 
    Save, Zap, Image as ImageIcon, X, ShieldCheck, DollarSign, 
    Layout, BookOpen, Plus, Brain, Loader2, Trash2, ChevronLeft,
    ChevronDown, ChevronRight, Sparkles, Experiment
} from './Icons';
import { Mission, MissionModule, MissionCategory } from '../types';
import { generateMissionStructure, generateImage, chatWithAI, AI_COSTS } from '../services/geminiService';
...
interface MissionFormProps {
  ...
  isSaving: boolean;
  onSave: (mission: Partial<Mission>, thumbnailFile: File | null, thumbnailBase64: string | null, qrFile: File | null) => void;
  onCancel: () => void;
}

const MissionForm: React.FC<MissionFormProps> = ({ initialMission, currentUserId, isSaving, onSave, onCancel }) => {
    ...
    return (
        <div className="max-w-4xl mx-auto animate-fade-in">
            <div className="flex justify-between items-center mb-6">
                <button onClick={onCancel} className="text-gray-500 font-bold flex items-center hover:text-gray-900 text-sm">
                    <ChevronLeft className="h-4 w-4 mr-1" /> ត្រឡប់ក្រោយ
                </button>
                <div className="flex gap-2">
                    <button onClick={() => setShowAiModal(true)} className="bg-purple-100 text-purple-700 px-4 py-2 rounded-lg font-bold flex items-center hover:bg-purple-200 transition-colors text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-1 space-y-6">
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        <div className="relative aspect-video bg-gray-100 rounded-xl overflow-hidden border border-gray-200 group mb-3">
                            {thumbnailBase64 || thumbnailFile || currentMission.thumbnail ? (
                                <img src={thumbnailBase64 ? `data:image/png;base64,${thumbnailBase64}` : (thumbnailFile ? URL.createObjectURL(thumbnailFile) : currentMission.thumbnail)} className="w-full h-full object-cover" />
                            ) : (
                            ...
                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                                <label className="bg-white text-gray-800 px-3 py-1.5 rounded-lg text-xs font-bold cursor-pointer hover:bg-gray-100">
                                    Upload
                                    <input type="file" className="hidden" accept="image/*" onChange={e => { setThumbnailFile(e.target.files?.[0] || null); setThumbnailBase64(null); }} />
                                </label>
                                <button onClick={handleGenerateThumbnail} className="bg-purple-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-purple-700 flex items-center">
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        {isOwner ? (
                            <>
                                {currentMission.mentorId ? (
                                    <div className="bg-green-50 p-3 rounded-xl border border-green-100 flex items-center justify-between">
                                        ...
                                        <button onClick={() => setCurrentMission({...currentMission, mentorId: undefined, mentor: 'សុភាទន្សាយ'})} className="text-red-400 hover:text-red-600 p-1">
                                            <X className="h-4 w-4" />
                                        </button>
                                    </div>
                                ) : (
                                    <div className="space-y-2">
                                        ...
                                        />
                                        <button onClick={handleAssignTeacher} disabled={assigningTeacher || !teacherEmail.trim()} className="w-full bg-gray-900 text-white py-2 rounded-lg text-xs font-bold hover:bg-black transition-colors disabled:opacity-50">
                                            {assigningTeacher ? <Loader2 className="h-3 w-3 animate-spin mx-auto" /> : 'ចាត់តាំង'}
                                        </button>
                                    </div>
                                )}
                            </>
                            ...
                            <div className="bg-gray-50 p-3 rounded-xl border border-gray-200">
                                ...
                                <p className="text-[10px] text-gray-400 mt-2 italic flex items-center"><X className="h-3 w-3 mr-1" /> Read Only</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 space-y-4">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1">ប្រភេទ (Category)</label>
                            <select 
                                className="w-full p-2 border border-gray-200 rounded-lg text-sm" 
                                value={currentMission.category} 
                                onChange={e => setCurrentMission({...currentMission, category: e.target.value as MissionCategory})}
                            >
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">ចំនួនសិស្សក្នុងក្រុម</label>
                                <input 
                                    type="number" 
                                    className="w-full p-2 border border-gray-200 rounded-lg text-sm" 
                                    value={currentMission.squadSize} 
                                    onChange={e => setCurrentMission({...currentMission, squadSize: parseInt(e.target.value)})} 
                                    min={1}
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">របៀបបង្កើតក្រុម</label>
                                <select 
                                    className="w-full p-2 border border-gray-200 rounded-lg text-sm" 
                                    value={currentMission.squadCreation} 
                                    onChange={e => setCurrentMission({...currentMission, squadCreation: e.target.value as any})}
                                >
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1">ប្រភេទការចុះឈ្មោះ</label>
                            <select 
                                className="w-full p-2 border border-gray-200 rounded-lg text-sm" 
                                value={currentMission.enrollmentType} 
                                onChange={e => setCurrentMission({...currentMission, enrollmentType: e.target.value as any})}
                            >
                        ...
                        <div className="pt-2 border-t border-gray-100">
                            <label className="flex items-center space-x-2 cursor-pointer">
                                <input
                                    ...
                                    onChange={e => setCurrentMission({...currentMission, enablePlagiarismCheck: e.target.checked})}
                                />
                                <span className="text-xs font-bold text-gray-700">Enable Plagiarism Check (AI)</span>
                            </label>
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">KHQR (សម្រាប់បង់ប្រាក់)</label>
                                {currentMission.paymentQrUrl ? (
                                    <div className="relative group w-24 h-24 mb-2">
                                        <img src={currentMission.paymentQrUrl} className="w-full h-full object-cover rounded-lg border border-gray-200" alt="QR" />
                                        <button onClick={() => setCurrentMission({...currentMission, paymentQrUrl: undefined})} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"><X className="h-3 w-3"/></button>
                                    </div>
                                ) : (
                                    <input 
                                        type="file" 
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    onChange={e => setCurrentMission({...currentMission, paymentInstruction: e.target.value})}
                                    placeholder="Ex: ABA 001 234 567"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">Telegram Group Link</label>
                                <input 
                                    className="w-full p-2 border border-gray-200 rounded-lg text-sm text-blue-600" 
                                    value={currentMission.telegramGroupLink || ''}
                                    onChange={e => setCurrentMission({...currentMission, telegramGroupLink: e.target.value})}
                ...
                <div className="md:col-span-2 space-y-6">
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select className="w-full p-3 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary/20" value={currentMission.level} onChange={e => setCurrentMission({...currentMission, level: e.target.value as any})}>
                            ...
                            <div>
                                ...
                                <input type="number" className="w-full p-3 border border-gray-200 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-primary/20" value={currentMission.price} onChange={e => setCurrentMission({...currentMission, price: parseFloat(e.target.value)})} min={0} />
                            </div>
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1">បរិយាយ (Description)</label>
                            <textarea 
                    ...
                    <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={addModule} className="bg-white text-gray-700 px-3 py-1.5 rounded-lg border border-gray-200 text-xs font-bold hover:bg-gray-100 flex items-center">
            ...
            {showAiModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-scale-in">
                        <h3 className="font-bold text-lg mb-4 flex items-center"><Brain className="h-5 w-5 mr-2 text-purple-600"/> AI Mission Architect</h3>
                        <p className="text-sm text-gray-600 mb-4">Describe the mission topic, and AI will generate the structure, modules, and thumbnail for you.</p>
                        <textarea className="w-full p-3 border border-gray-200 rounded-xl text-sm h-24 mb-4 focus:ring-2 focus:ring-purple-200 outline-none" placeholder="e.g. A comprehensive guide to Python for Data Science..." value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} />
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setShowAiModal(false)} className="px-4 py-2 text-gray-500 font-bold text-sm">Cancel</button>
                            <button onClick={handleAiGenerateStructure} disabled={isGenerating || !aiPrompt.trim()} className="px-4 py-2 bg-purple-600 text-white rounded-lg font-bold text-sm shadow-lg disabled:opacity-50">
                                {isGenerating ? <Loader2 className="h-4 w-4 animate-spin" /> : `Generate Mission (${AI_COSTS.STRUCTURE} Pts)`}
                            </button>
                        </div>
                    </div>
                </div>
...
const ModuleEditor: React.FC<{
    ...
    const handleAutoGenerate = async () => {
        ...
        try {
            ...
            const response = await chatWithAI(prompt, [], "You are an expert curriculum designer. Return valid JSON only.");
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-4 transition-all">
            ...
            >
                <div className="flex items-center gap-3">
                    <div className={`p-1 rounded-full ${isExpanded ? 'bg-primary/10 text-primary' : 'bg-gray-100 text-gray-400'}`}>
                        {isExpanded ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                    </div>
                    <div>
                        <span className="font-bold text-gray-500 text-xs uppercase block">Module {index + 1}</span>
                        <span className="font-bold text-gray-900 text-sm">{module.title || 'Untitled Module'}</span>
                    </div>
                </div>
            ...
            {isExpanded && (
                <div className="space-y-4 mt-4 pt-4 border-t border-gray-100 animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-400 mb-1 uppercase">AI Persona</label>
                            <textarea 
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-400 mb-1 uppercase">Initial Greeting</label>
                            <textarea 
                                className="w-full p-3 border border-gray-200 rounded-lg text-xs focus:ring-2 focus:ring-primary/20 outline-none resize-none min-h-[120px]"
                    ...
                    <div>
                        <label className="block text-xs font-bold text-gray-400 mb-1 uppercase">Theory Prompt</label>
                        <textarea 
                    ...
                    <div className="mt-4 border-t border-gray-100 pt-4">
                        ...
                        <div className="flex gap-2">
                            ...
                            <input 
                                ...
                                placeholder={module.simulationConfig?.type === 'wokwi' ? "https://wokwi.com/projects/..." : "https://phet.colorado.edu/..."}
                        ...
                        {module.simulationConfig?.type === 'wokwi' && (
                            <p className="text-[10px] text-gray-400 mt-1 italic">
                                Paste the Wokwi Project URL (e.g. https://wokwi.com/projects/305568836183138882). It will auto-convert to embed.
                            </p>
                        )}
                    </div>
                </div>
