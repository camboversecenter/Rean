...
import { Zap, Mic, Camera, Heart, MessageCircle, CheckCircle, Award, Send, X, Loader2, ChevronDown, Ghost, User, Search, ChevronRight, Bookmark, Clock } from './Icons';
import { StudentPost, CommunityReply, ReactionType } from '../types';
import { chatWithAI, AI_COSTS } from '../services/geminiService';
import { fetchCommunityFeed, createStudentPost, createCommunityReply, toggleReaction, SortOption } from '../services/communityService';
import { toggleBookmark, fetchBookmarkedPostIds, fetchSavedPosts } from '../services/bookmarkService';
...
const CommunityFeed: React.FC = () => {
  ...
  const handleBookmark = async (post: StudentPost) => {
      ...
      try {
          ...
          setBookmarkedPosts(prev => {
            const newSet = new Set(prev);
            if (isBookmarked) newSet.add(post.id); else newSet.delete(post.id);
            return newSet;
        });
      }
  };
           ...
           <div className="px-4 mb-4 flex items-center justify-between">
              ...
              {userProfile && (
                  <div className="bg-white border border-gray-200 px-3 py-1 rounded-full flex items-center shadow-sm">
                      <div className="w-4 h-4 rounded-full bg-secondary flex items-center justify-center text-[8px] text-white font-bold mr-2">P</div>
                      <span className="text-xs font-bold text-gray-700">{userProfile.spendable_points || 0} ពិន្ទុ</span>
                  </div>
              )}
           </div>
              ...
              <div className="flex gap-3 mb-1">
                  ...
                  <div className="flex-1">
                    <textarea 
                      ...
                      placeholder={isAnonymous ? "សួរដោយអនាមិក (Ninja Mode)..." : "តើអ្នកចង់ដឹងអ្វីនៅពេលនេះ?"}
              ...
              <div className="flex flex-wrap justify-between items-center mt-2 pt-2 border-t border-gray-50 gap-2">
                  <div className="flex items-center space-x-2">
                      ...
                      >
                        ...
                        <span className="hidden sm:inline">{isAnonymous ? 'អនាមិក (On)' : 'អនាមិក (Off)'}</span>
                      </button>
                  </div>
                  ...
                  >
                    {isPosting ? 'កំពុងគិត...' : <><Zap className="h-4 w-4 mr-1 fill-white" /> សួរភ្លាមៗ ({AI_COSTS.CHAT} Pt)</>}
                  </button>
              </div>
           </div>
           )}
               ...
               {activeTab === 'all' && (
                   <div className="flex items-center space-x-2 overflow-x-auto scrollbar-hide pb-2">
                       <button
                           ...
                           className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all border ${
                               sortBy === 'latest' ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-white text-gray-600 border-gray-200'
                           }`}
                       >
                       ...
                       <button
                           ...
                           className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all border ${
                               sortBy === 'trending' ? 'bg-orange-100 text-orange-700 border-orange-200' : 'bg-white text-gray-600 border-gray-200'
                           }`}
                       >
                           <Zap className={`h-3.5 w-3.5 ${sortBy === 'trending' ? 'fill-orange-600' : ''}`} /> កំពុងពេញនិយម (Trending)
                       </button>
                       <button
                           onClick={() => setSortBy('bounty')}
                           className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all border ${
                               sortBy === 'bounty' ? 'bg-yellow-100 text-yellow-800 border-yellow-200' : 'bg-white text-gray-600 border-gray-200'
                           }`}
                       >
              ...
              {isLoading ? (
                  ...
                  <>
                    ...
                    {posts.map(post => {
                        ...
                        const displayAuthor = isPostAnonymous ? 'សិស្សអនាមិក (Anonymous Student)' : post.authorName;
                        ...
                        return (
                            <div key={post.id} onClick={() => navigate(`/community/question/${post.id}`)} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 animate-fade-in relative overflow-hidden cursor-pointer hover:shadow-md transition-shadow group h-full flex flex-col">
                                {(post.bounty_points || 0) > 0 && (
                                    <div className={`absolute top-0 right-0 text-white text-[10px] font-bold px-2 py-1 rounded-bl-xl shadow-sm z-10 ${
                                ...
                                <div className="flex items-start gap-3 mb-2">
                                    ...
                                    <div className="flex-1">
                                        <div className="flex justify-between items-start">
                                            <h3 className={`text-sm font-bold ${isPostAnonymous ? 'text-gray-500 italic' : 'text-gray-900'}`}>
                                                {displayAuthor}
                                            </h3>
                                ...
                                <div key={reply.id} className={`ml-8 mt-2 pl-3 py-1.5 border-l-2 relative ${
                                    reply.accepted ? 'border-green-500 bg-green-50/30 rounded-r-lg' : 
                                    reply.isAI ? 'border-primary bg-primary/5 rounded-r-lg' : 
                                    'border-gray-200'
                                }`}>
                                    <div className="flex items-center gap-1 mb-1">
                                        {reply.isAI ? (
                                            ...
                                            <img src={reply.authorAvatar || `https://ui-avatars.com/api/?name=${reply.authorName}`} className="w-4 h-4 rounded-full" alt="avt" />
                                        )}
                                        <span className={`text-[10px] font-bold ${reply.isAI ? 'text-primary' : 'text-gray-700'}`}>
                                            {reply.isAI && reply.authorName === 'Kru REAN' ? 'សុភាទន្សាយ' : reply.authorName}
                                        </span>
                                        {reply.accepted && (
