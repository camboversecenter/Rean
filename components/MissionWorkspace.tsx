...
import { 
  CheckCircle, Bot, Send, Users2, FileText, Brain, Target, ChevronRight, Lock, PlayCircle, AlertCircle, Loader2, Award, MessageCircle, Edit, Save, BookOpen, Zap, Globe, Copy, ShieldCheck, Home, Camera, Image as ImageIcon, Terminal, Layout, X, Experiment
} from './Icons';
import { Mission, ChatMessage, MissionModuleStatus, SquadMember } from '../types';
import { chatWithAI, evaluateSubmission, evaluateImageSubmission, AI_COSTS } from '../services/geminiService';
import { updateMissionProgress, updateMissionSquadNote, getSquadMembers, checkPlagiarism, saveSubmissionVector, updateEnrollmentStatus } from '../services/missionProgressService';
...
interface MissionWorkspaceProps {
  ...
  initialProgressDetails?: Record<string, { status: MissionModuleStatus, submission: string, feedback: string }>;
...
const MissionWorkspace: React.FC<MissionWorkspaceProps> = ({ mission, enrollmentId, initialProgress, initialProgressDetails, initialSquadNote, squadId }) => {
  const navigate = useNavigate();
  const [activeModuleId, setActiveModuleId] = useState<string>(() => {
      if (!initialProgress) return mission.modules[0].id;
      const firstActive = mission.modules.find(m => {
          const status = initialProgress[m.id];
          return status === 'active' || !status;
      });
      return firstActive ? firstActive.id : mission.modules[0].id;
  });

  const [moduleStatus, setModuleStatus] = useState<Record<string, MissionModuleStatus>>(() => {
    if (initialProgress && Object.keys(initialProgress).length > 0) {
        return initialProgress;
    }
    const initial: Record<string, MissionModuleStatus> = {};
    mission.modules.forEach((m, idx) => {
        initial[m.id] = idx === 0 ? 'active' : 'locked';
    });
    return initial;
  });

  // Added 'simulation' to activeTab types
  const [activeTab, setActiveTab] = useState<'brief' | 'learn' | 'studio' | 'team' | 'simulation'>('brief');
  ...
  const [codeLanguage, setCodeLanguage] = useState<'javascript' | 'python' | 'html'>('javascript');
  const [submissionImage, setSubmissionImage] = useState<string | null>(null); // Base64
  
  const [evaluationData, setEvaluationData] = useState<Record<string, {passed: boolean, score?: number, feedback: string} | null>>({});

  const [generatedLessons, setGeneratedLessons] = useState<Record<string, string>>({});
  const [loadingLesson, setLoadingLesson] = useState(false);
  const [lessonLanguage, setLessonLanguage] = useState<'km' | 'en'>('km');

  const [squadNote, setSquadNote] = useState(initialSquadNote || '');
  ...
  useEffect(() => {
      if (activeModule.simulationConfig && activeTab !== 'simulation' && !initialProgressDetails?.[activeModuleId]) {
  ...
  const getTheoryPromptText = () => {
      return activeModule.theoryPrompt || `Explain the core concepts required to complete this task: "${activeModule.task}". Provide examples.`;
  };
  ...
  const handleSubmitWork = async () => {
      if ((!currentSubmissionText.trim() && !submissionImage) || currentSubmissionText.length > SUBMISSION_LIMIT) {
      ...
      try {
          ...
          if (enrollmentId && mission.enablePlagiarismCheck && currentSubmissionText.length > 50) {
              const similarSubmission = await checkPlagiarism(mission.id, activeModuleId, enrollmentId, currentSubmissionText);
              if (similarSubmission && similarSubmission.similarity > 0.85) {
                  toast.error(`⚠️ រកឃើញភាពដូចគ្នាខ្លាំងទៅនឹងចម្លើយដែលមានស្រាប់ (${(similarSubmission.similarity * 100).toFixed(0)}%)។ សូមសរសេរដោយខ្លួនឯង។`);
          ...
          if (submissionImage) {
              ...
              result = await evaluateSubmission(activeModule.task, activeModule.aiPersona, currentSubmissionText);
          }
          ...
          if (result.passed) {
              const newStatusMap: Record<string, MissionModuleStatus> = {...moduleStatus, [activeModuleId]: 'completed'};
              ...
              if (enrollmentId) {
                  ...
                  if (submissionImage) {
                      ...
                      if (oldSubmission) {
                          const urlMatch = oldSubmission.match(/https:\/\/[^\s]+\.supabase\.co\/storage\/v1\/object\/public\/[^\s]+/);
                  ...
                  await updateMissionProgress(enrollmentId, activeModuleId, 'completed', finalSubmissionText, persistentFeedback);
                  
                  if (mission.enablePlagiarismCheck && currentSubmissionText) {
                      await saveSubmissionVector(mission.id, enrollmentId, activeModuleId, currentSubmissionText);
                  }
              ...
              if (enrollmentId) {
                  await updateMissionProgress(enrollmentId, activeModuleId, 'active', currentSubmissionText, persistentFeedback);
              }
          }
  ...
  const handleStartLesson = async () => {
      ...
      const langInstruction = lessonLanguage === 'km' 
    ...
    <div className="flex flex-col h-[calc(100vh-64px)] bg-gray-100 overflow-hidden relative">
        ...
        <div className="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-4 shadow-sm z-20">
            <div>
                ...
                <p className="text-xs text-gray-500">ការគ្រប់គ្រងបេសកកម្ម • {mission.modules.filter(m => moduleStatus[m.id] === 'completed').length} / {mission.modules.length} បានបញ្ចប់</p>
            </div>
            <div className="flex items-center gap-2">
                {mission.telegramGroupLink && (
                    <a href={mission.telegramGroupLink} target="_blank" rel="noreferrer" className="flex items-center text-xs font-bold text-white bg-blue-500 px-3 py-1.5 rounded-lg hover:bg-blue-600 transition-colors shadow-sm">
        ...
        <div className="flex flex-1 overflow-hidden">
            <div className="w-20 md:w-64 bg-white border-r border-gray-200 flex-shrink-0 flex flex-col overflow-y-auto hidden-scrollbar">
                <div className="p-4 pb-20 md:pb-4">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 hidden md:block">ផែនទីបេសកកម្ម</h3>
                    <div className="space-y-2 relative">
                        ...
                        {mission.modules.map((mod, idx) => {
                            ...
                            if (status === 'completed') {
                                circleClass = 'bg-green-500 border-green-500 text-white shadow-sm shadow-green-200';
                                icon = <CheckCircle className="h-4 w-4" />;
                            } else if (status === 'active') {
                                circleClass = 'bg-white border-primary text-primary shadow-sm ring-2 ring-primary/10';
                            ...
                            return (
                                ...
                                >
                                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mr-3 border-2 transition-colors duration-200 ${circleClass}`}>
                                        {icon}
                                    </div>
                                    <div className="hidden md:block">
                                        <p className={`text-sm font-bold transition-colors line-clamp-1 ${isActive ? 'text-primary' : 'text-gray-600 group-hover:text-gray-900'}`}>{mod.title}</p>
                                        <p className="text-[10px] text-gray-400 capitalize">{status === 'completed' ? 'បានបញ្ចប់' : status === 'active' ? 'កំពុងដំណើរការ' : 'ចាក់សោ'}</p>
                                    </div>
                                </button>
                            );
                        })}
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-gray-50">
                <div className="flex bg-white border-b border-gray-200 px-4 overflow-x-auto scrollbar-hide">
                    <button onClick={() => setActiveTab('brief')} className={`py-3 px-4 text-sm font-bold border-b-2 transition-colors flex items-center whitespace-nowrap ${activeTab === 'brief' ? 'border-primary text-primary' : 'border-transparent text-gray-500'}`}><Target className="h-4 w-4 mr-2" /> សង្ខេប</button>
                    
                    {/* Add Simulation Tab conditionally */}
                    {activeModule.simulationConfig && (
                        <button onClick={() => setActiveTab('simulation')} className={`py-3 px-4 text-sm font-bold border-b-2 transition-colors flex items-center whitespace-nowrap ${activeTab === 'simulation' ? 'border-primary text-primary' : 'border-transparent text-gray-500'}`}>
                            <Experiment className="h-4 w-4 mr-2" /> {activeModule.simulationConfig.type === 'wokwi' ? 'IoT Lab (Wokwi)' : 'ពិសោធន៍ (Sim)'}
                        </button>
                    )}

                    <button onClick={() => setActiveTab('learn')} className={`py-3 px-4 text-sm font-bold border-b-2 transition-colors flex items-center whitespace-nowrap ${activeTab === 'learn' ? 'border-primary text-primary' : 'border-transparent text-gray-500'}`}><BookOpen className="h-4 w-4 mr-2" /> រៀន</button>
                    <button onClick={() => setActiveTab('studio')} className={`py-3 px-4 text-sm font-bold border-b-2 transition-colors flex items-center whitespace-nowrap ${activeTab === 'studio' ? 'border-primary text-primary' : 'border-transparent text-gray-500'}`}><Edit className="h-4 w-4 mr-2" /> កន្លែងអនុវត្ត</button>
                    <button onClick={() => setActiveTab('team')} className={`py-3 px-4 text-sm font-bold border-b-2 transition-colors flex items-center whitespace-nowrap ${activeTab === 'team' ? 'border-primary text-primary' : 'border-transparent text-gray-500'}`}><Users2 className="h-4 w-4 mr-2" /> បន្ទប់ក្រុម</button>
                </div>
                        ...
                        <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
                            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                                ...
                                <div className="bg-blue-50 text-blue-800 p-4 rounded-xl text-sm font-medium mb-6 flex items-start">
                                    ...
                                    <div>
                                        <span className="block font-bold mb-1 uppercase text-xs tracking-wider">គោលបំណង</span>
                                        <MarkdownText content={activeModule.task} />
                                    </div>
                                </div>
                                <div className="prose prose-sm max-w-none text-gray-600">
                                    <p>ស្វាគមន៍មកកាន់មេរៀននេះ។ 
                                    {activeModule.simulationConfig ? (
                                        <> សូមចូលទៅកាន់ផ្ទាំង <strong>{activeModule.simulationConfig.type === 'wokwi' ? 'IoT Lab' : 'ពិសោធន៍ (Sim)'}</strong> ដើម្បីចាប់ផ្តើម។</>
                                    ) : (
                                        <> ប្រើប្រាស់ផ្ទាំង <strong>កន្លែងអនុវត្ត (Studio)</strong> ដើម្បីដាក់ស្នើកិច្ចការរបស់អ្នក។</>
                                    )}
                                    </p>
                                </div>
                            </div>
                            <div className="flex justify-end gap-3">
                                <button onClick={() => setActiveTab('learn')} className="bg-white border border-gray-200 text-gray-700 px-6 py-3 rounded-xl font-bold flex items-center hover:bg-gray-50 transition-colors"><BookOpen className="h-4 w-4 mr-2" /> រៀនសិន</button>
                                {activeModule.simulationConfig ? (
                                    <button onClick={() => setActiveTab('simulation')} className="bg-primary text-white px-6 py-3 rounded-xl font-bold shadow-lg flex items-center hover:scale-105 transition-transform"><Experiment className="h-4 w-4 mr-2" /> ចាប់ផ្តើមពិសោធន៍</button>
                                ) : (
                                    <button onClick={() => setActiveTab('studio')} className="bg-primary text-white px-6 py-3 rounded-xl font-bold shadow-lg flex items-center hover:scale-105 transition-transform">ចាប់ផ្តើមអនុវត្ត <ChevronRight className="h-4 w-4 ml-2" /></button>
                                )}
                            </div>
                        </div>
                    )}
                        ...
                        <div className="flex flex-col h-full gap-4 animate-fade-in">
                            <div className="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden relative min-h-[500px]">
                                {activeModule.simulationConfig.type === 'wokwi' ? (
                                    ...
                                    <iframe 
                                        src={activeModule.simulationConfig.url.replace(/\/embedded\/?$/, '').replace(/\/$/, '') + (activeModule.simulationConfig.url.includes('embed=1') ? '' : '?embed=1')}
                            ...
                            <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
                                ...
                                <div className="flex flex-col sm:flex-row gap-4 items-start">
                                    <div className="flex-1 text-sm text-gray-600">
                                        ...
                                        <p className="mb-2">២. ថតរូបអេក្រង់ (Screenshot) លទ្ធផលនៃការពិសោធន៍របស់អ្នក។</p>
                                            ...
                                            onChange={(e) => {
                                                if (e.target.files?.[0]) {
                                                    ...
                                                    reader.onloadend = () => {
                                                        ...
                                                        setActiveTab('studio');
                                                        toast.success("រូបភាពត្រូវបានជ្រើសរើស! សូមចុច Submit នៅផ្ទាំង Studio។");
                                                    };
                            ...
                            {generatedLessons[activeModuleId] ? (
                                <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100">
                                    <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-4">
                                        <h3 className="font-bold text-gray-900">មេរៀន ({lessonLanguage === 'km' ? 'ភាសាខ្មែរ' : 'English'})</h3>
                                        <button onClick={() => setGeneratedLessons(prev => ({...prev, [activeModuleId]: ''}))} className="text-sm text-primary font-bold hover:underline">បង្កើតថ្មី</button>
                                    </div>
                                    <div className="prose prose-blue max-w-none"><MarkdownText content={generatedLessons[activeModuleId]} /></div>
                                    <div className="mt-8 pt-6 border-t border-gray-100 flex justify-end">
                                        <button onClick={() => setActiveTab('studio')} className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors">ខ្ញុំត្រៀមខ្លួនអនុវត្តហើយ</button>
                                    </div>
                                </div>
                            ) : (
                                <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-white rounded-2xl shadow-sm border border-gray-100">
                                    <div className="w-20 h-20 bg-indigo-50 rounded-full flex items-center justify-center mb-6"><Brain className="h-10 w-10 text-indigo-500" /></div>
                                    <h2 className="text-2xl font-bold text-gray-900 mb-2">ដោះសោចំណេះដឹង</h2>
                                    <p className="text-gray-500 mb-6 max-w-md">ជ្រើសរើសភាសាដែលអ្នកចង់សិក្សា ហើយ AI នឹងបង្កើតមេរៀនសង្ខេបសម្រាប់អ្នក។</p>
                                    ...
                                    <button onClick={handleStartLesson} disabled={loadingLesson} className="bg-indigo-600 text-white px-8 py-4 rounded-xl font-bold shadow-xl flex items-center hover:scale-105 transition-transform">
                                        {loadingLesson ? <Loader2 className="h-5 w-5 animate-spin mr-2" /> : <Zap className="h-5 w-5 mr-2 fill-white" />}
                                        {loadingLesson ? 'កំពុងបង្កើតមេរៀន...' : `បង្កើតមេរៀន (${AI_COSTS.LESSON} Pts)`}
                                    </button>
                                </div>
                            )}
                        </div>
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
                                ...
                                <div className="px-4 pt-4">
                                    {submissionImage ? (
                                        <div className="relative w-full h-48 bg-gray-100 rounded-xl overflow-hidden border border-gray-200 mb-2">
                                            <img src={submissionImage} alt="Homework" className="w-full h-full object-contain" />
                                            <button 
                                        ...
                                        >
                                            ...
                                            <span className="text-xs text-gray-500 font-medium">Attach Image / Screenshot (Optional)</span>
                                            <input 
                                                type="file" 
                                                accept="image/*" 
                                                ref={fileInputRef}
                                ...
                                <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center">
                                    <span className="text-xs text-gray-400">
                                        {submissionType === 'code' ? 'Code is run in browser sandbox' : 'Supports Markdown & LaTeX $$...$$'}
                                    </span>
                                    <button 
                                        onClick={handleSubmitWork}
                                        disabled={isEvaluating || moduleStatus[activeModuleId] === 'completed' || (!currentSubmissionText.trim() && !submissionImage) || currentSubmissionText.length > SUBMISSION_LIMIT}
                                        className={`px-6 py-2.5 rounded-xl font-bold text-sm shadow-md transition-all flex items-center ${
                                            moduleStatus[activeModuleId] === 'completed' ? 'bg-green-500 text-white' : 
                                            isFailedAttempt ? 'bg-orange-500 text-white hover:bg-orange-600' : 'bg-gray-900 text-white'
                                        } disabled:opacity-50`}
                                    >
                                        {isEvaluating ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <CheckCircle className="h-4 w-4 mr-2" />}
                                        {isEvaluating ? 'Checking...' : 
                                         moduleStatus[activeModuleId] === 'completed' ? 'បានបញ្ចប់ (Completed)' : 
                                         isFailedAttempt ? `Retry (${submissionImage ? AI_COSTS.EVALUATION + 5 : AI_COSTS.EVALUATION} Pts)` :
                            ...
                            <div className="flex gap-4 items-start animate-fade-in">
                                ...
                                <div className="bg-white p-4 rounded-2xl rounded-tl-none shadow-sm border border-gray-100 flex-1">
                                    ...
                                    <div className="text-sm text-gray-700 leading-relaxed">
                                        <MarkdownText content={activeModule.initialPrompt || "សួស្តី! តោះចាប់ផ្តើមបេសកកម្មនេះទាំងអស់គ្នា។"} />
                                    </div>
                                </div>
                            </div>

                            {currentEvaluation && (
                                <div className={`p-4 rounded-xl border ${currentEvaluation.passed ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} animate-scale-in`}>
                                    <div className="flex items-start gap-4">
                                        <div className="flex-1 min-w-0">
                                            <h4 className={`font-bold text-sm mb-2 ${currentEvaluation.passed ? 'text-green-800' : 'text-red-800'}`}>{currentEvaluation.passed ? 'ល្អណាស់!' : 'ត្រូវការកែតម្រូវ (Needs Improvement)'}</h4>
                                            <div className="text-sm leading-relaxed"><MarkdownText content={currentEvaluation.feedback} /></div>
                                        </div>
                                    </div>
                                </div>
                            )}
                    ...
                    {activeTab === 'team' && (
                        <div className="max-w-3xl mx-auto h-full animate-fade-in flex flex-col gap-4">
                            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
                                <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">សមាជិកក្រុម ({squadMembers.length})</h3>
                                <div className="flex flex-wrap gap-4">{squadMembers.map(member => (<div key={member.studentId} className="flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg border border-gray-100"><span className="text-sm font-bold text-gray-800">{member.fullName}</span></div>))}</div>
                            </div>
                            ...
                            <div className="bg-yellow-50 rounded-2xl shadow-sm border border-yellow-100 flex-1 flex flex-col min-h-[300px]">
                                <div className="p-4 border-b border-yellow-100 flex items-center justify-between">
                                    <div className="flex items-center gap-2"><FileText className="h-5 w-5 text-yellow-600" /><h3 className="font-bold text-yellow-900">កំណត់ហេតុក្រុម (Squad Note)</h3></div>
                                    <div className="flex items-center gap-2">
                                        {isSaving ? <span className="text-xs text-yellow-600 animate-pulse">កំពុងរក្សាទុក...</span> : <span className="text-xs text-yellow-600">បានរក្សាទុក {lastSaved ? lastSaved.toLocaleTimeString() : ''}</span>}
                                    </div>
                                </div>
            ...
            <div className="hidden lg:flex w-80 bg-white border-l border-gray-200 flex-col">
                <div className="h-14 border-b border-gray-100 flex items-center px-4 bg-primary/5">
                    ...
                    <div><h3 className="text-sm font-bold text-gray-900">គ្រូជំនួយ AI</h3></div>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50/50">
                    {(messages[activeModuleId] || []).map((msg) => (
                        <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[85%] rounded-2xl p-3 text-sm leading-relaxed shadow-sm ${msg.role === 'user' ? 'bg-white text-gray-800 border' : 'bg-primary text-white'}`}><MarkdownText content={msg.text} /></div></div>
                    ))}
                    <div ref={messagesEndRef} />
                </div>
                <div className="p-3 border-t border-gray-100 bg-white">
                    <div className="relative">
                        <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="សួរសំណួរ... (1 Pt)" className="w-full bg-gray-100 text-sm rounded-xl pl-3 pr-10 py-3 focus:outline-none" />
                        <button onClick={handleSendMessage} disabled={!chatInput.trim() || isChatLoading} className="absolute right-2 top-1.5 p-1.5 bg-primary text-white rounded-lg disabled:opacity-50"><Send className="h-4 w-4" /></button>
                    </div>
                </div>
            </div>
        </div>
