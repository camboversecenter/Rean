...
const SchoolCard: React.FC<SchoolCardProps> = ({ school }) => {
  ...
  const activeAdmissions = school.admissions.filter(a => a.status === 'Open' || a.status === 'Closing Soon');
  ...
  return (
    <Link to={`/school/${school.id}`} className="block bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-4 hover:shadow-md transition-shadow group">
      <div className="relative h-32 bg-gray-200">
        <img src={school.coverImage || 'https://via.placeholder.com/800x400'} alt={school.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        <div className="absolute top-2 right-2 flex gap-1">
             {/* Status Badge */}
             <div className={`px-2 py-1 rounded-full text-[10px] font-bold flex items-center shadow-sm ${
                statusLabel === 'កំពុងទទួល' ? 'bg-green-500 text-white' : 
                statusLabel === 'ជិតចប់' ? 'bg-orange-500 text-white' : 
      ...
      <div className="p-4 relative">
        ...
        <div className="absolute -top-10 left-4 bg-white p-1 rounded-xl shadow-md">
            <img src={school.logo || 'https://via.placeholder.com/150'} alt="logo" className="w-16 h-16 rounded-lg object-contain bg-white" />
        </div>
        ...
        <div className="mt-8">
            ...
            {primaryAdmission ? (
                <div className="bg-blue-50 border border-blue-100 rounded-lg p-2.5 mb-3 mt-2">
                    ...
                    <p className="text-xs text-blue-600 line-clamp-1">{primaryAdmission.description || 'ទទួលពាក្យសម្រាប់ឆ្នាំសិក្សាថ្មី'}</p>
