...
import { Loader2, Target, BookOpen, CheckCircle, GraduationCap, ChevronLeft, Award } from '../components/Icons';
...
const PublicProfilePage: React.FC = () => {
    ...
    // Filter Lists
    const activeMissions = missions.filter(m => m.enrollmentStatus === 'In Progress' || m.enrollmentStatus === 'Pending');
    const completedMissions = missions.filter(m => m.enrollmentStatus === 'Completed');
    
    const activeCourses = courses.filter(c => c.enrollmentStatus === 'Pending' || c.enrollmentStatus === 'Approved');
    const completedCourses = courses.filter(c => c.enrollmentStatus === 'Completed');

    // Combine for display based on tab
    const displayActive = [...activeMissions.map(m => ({...m, type: 'mission'})), ...activeCourses.map(c => ({...c, type: 'course'}))];
    const displayCompleted = [...completedMissions.map(m => ({...m, type: 'mission'})), ...completedCourses.map(c => ({...c, type: 'course'}))];
        ...
        <div className="min-h-screen bg-gray-50 pb-20 font-sans animate-fade-in">
            ...
            <div className="bg-white border-b border-gray-200 mb-8 relative">
                ...
                <div className="h-32 md:h-48 bg-gradient-to-r from-blue-500 to-teal-400 relative z-0">
                    <button onClick={() => navigate(-1)} className="absolute top-4 left-4 p-2 bg-white/20 backdrop-blur-md rounded-full text-white hover:bg-white/30 transition-colors z-30">
                ...
                <div className="max-w-5xl mx-auto px-4 pb-6 relative z-10">
                    <div className="flex flex-col md:flex-row items-center md:items-end -mt-12 md:-mt-16 gap-4 md:gap-6">
                        <div className="w-24 h-24 md:w-36 md:h-36 rounded-full border-4 border-white shadow-lg overflow-hidden bg-gray-200 flex-shrink-0">
                            <img src={profile.avatar_url || `https://ui-avatars.com/api/?name=${profile.full_name}`} className="w-full h-full object-cover" alt="Profile" />
                        </div>
                        <div className="text-center md:text-left flex-1 mb-2 md:mb-4">
                            ...
                            <div className="flex items-center justify-center md:justify-start gap-2">
                                <span className="bg-gray-100 text-gray-600 text-[10px] px-2 py-0.5 rounded uppercase font-bold border border-gray-200">{profile.role || 'Student'}</span>
                                <span className="bg-yellow-100 text-yellow-700 text-[10px] px-2 py-0.5 rounded font-bold border border-yellow-200">Lvl {level}</span>
                            </div>
                        </div>
                                ...
                                <div key={`c-${idx}`} className="relative">
                                    ...
                                    <div className="absolute top-2 right-2">
                                        <span className={`px-2 py-1 rounded text-[10px] font-bold shadow-sm ${
                                            item.enrollmentStatus === 'Completed' ? 'bg-green-500 text-white' : 'bg-yellow-500 text-white'
                                        }`}>
