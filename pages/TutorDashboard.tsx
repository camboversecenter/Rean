...
import { 
    GraduationCap, Save, Loader2, Briefcase, 
    Calendar, CheckCircle, X, User, MapPin, 
    DollarSign, Clock, Phone, AlertCircle, Send,
    BookOpen, Video, Filter, ChevronRight, FileText,
    Eye, EyeOff, Camera, Zap, Image as ImageIcon
} from '../components/Icons';
import { 
    getMyTutorProfile, updateTutorProfile, 
    getTutorBookings, updateBookingStatus, 
    fetchStudentRequests, applyToRequest 
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            const updatedProfile = {
                ...
                grades: gradesInput.split(',').map(s => s.trim()).filter(s => s),
                coverImage: coverUrl
            };
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-24">
            ...
            <div className="container mx-auto px-4 py-6 max-w-5xl">
                ...
                <div className={`mb-8 p-5 rounded-3xl border-2 transition-all duration-300 flex flex-col md:flex-row justify-between items-center gap-4 ${
                    profile.is_listed 
                    ? 'bg-green-50 border-green-100 shadow-sm' 
                    : 'bg-amber-50 border-amber-100 shadow-sm'
                }`}>
                    <div className="flex items-center gap-4">
                        <div className={`p-3 rounded-2xl ${profile.is_listed ? 'bg-green-500 text-white' : 'bg-amber-500 text-white shadow-inner'}`}>
                    ...
                    <div className="flex items-center gap-3 bg-white/50 p-2 rounded-2xl border border-white/50 backdrop-blur-sm">
                        <span className={`text-xs font-bold ${profile.is_listed ? 'text-gray-400' : 'text-amber-600'}`}>លាក់</span>
                        <button 
                        ...
                        <span className={`text-xs font-bold ${profile.is_listed ? 'text-green-600' : 'text-gray-400'}`}>ផ្សាយ</span>
                    </div>
                </div>
                ...
                <div className="animate-fade-in">
                                ...
                                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
                                    <div className="relative inline-block mb-4">
                                        <div className="w-24 h-24 rounded-full bg-gray-100 p-1 border border-gray-200 mx-auto">
                                            <img src={profile.avatarUrl || `https://ui-avatars.com/api/?name=${profile.fullName}`} alt="avatar" className="w-full h-full object-cover rounded-full"/>
                                        </div>
                                    </div>
                                    <h3 className="font-bold text-lg text-gray-900">{profile.fullName}</h3>
                                    <p className="text-xs text-gray-500 mb-4">{profile.id ? 'Verified Tutor' : 'New Tutor'}</p>
                                    
                                    <div className="space-y-4 text-left">
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 mb-1">តម្លៃម៉ោង (Riel)</label>
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 mb-1">លេខទូរស័ព្ទ</label>
                                            <div className="relative">
                            ...
                            <div className="lg:col-span-2 space-y-6">
                                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                    <div className="flex justify-between items-center mb-4">
                                        ...
                                        >
                                            {isGeneratingCover ? <Loader2 className="h-3 w-3 animate-spin mr-1"/> : <Zap className="h-3 w-3 mr-1"/>}
                                            AI Generate
                                        </button>
                                    </div>
                                    <div className="relative aspect-[3/1] bg-gray-100 rounded-xl overflow-hidden border border-dashed border-gray-300 flex items-center justify-center group cursor-pointer">
                                        {coverBase64 || coverFile || profile.coverImage ? (
                                            <img 
                                                src={coverBase64 ? `data:image/png;base64,${coverBase64}` : (coverFile ? URL.createObjectURL(coverFile) : profile.coverImage)} 
                                ...
                                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 mb-1">មុខវិជ្ជា (Subjects)</label>
                                            <input 
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-gray-500 mb-1">កម្រិត (Grades)</label>
                                            <input 
                                                className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" 
                                                value={gradesInput} 
                                    ...
                                    <div className="mb-4">
                                        <label className="block text-xs font-bold text-gray-500 mb-1">ទម្រង់បង្រៀន</label>
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-gray-500 mb-1">បទពិសោធន៍ (Experience)</label>
                                        <input 
                                            className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary/20" 
                    ...
                    {activeTab === 'bookings' && (
                        <div className="space-y-8">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {pendingBookings.map(b => (
                                            <div key={b.id} className="bg-white rounded-2xl shadow-sm border-l-4 border-orange-400 p-5 hover:shadow-md transition-shadow">
                                                <div className="flex justify-between items-start mb-3">
                                                    <div className="flex items-center gap-3">
                                                        <img src={b.studentAvatar || `https://ui-avatars.com/api/?name=${b.studentName}`} className="w-10 h-10 rounded-full bg-gray-200" />
                                                        <div>
                                                    ...
                                                    <span className="bg-orange-100 text-orange-700 text-[10px] font-bold px-2 py-1 rounded">New Request</span>
                                                </div>
                                                
                                                <div className="bg-gray-50 p-3 rounded-xl mb-4 text-sm text-gray-700 space-y-1">
                                                    <p><span className="font-bold text-gray-500 text-xs uppercase w-20 inline-block">Subject:</span> {b.subject}</p>
                                                    <p><span className="font-bold text-gray-500 text-xs uppercase w-20 inline-block">Time:</span> {b.scheduledTime}</p>
                                                    {b.locationNotes && <p className="italic text-gray-500 border-t border-gray-200 pt-2 mt-2">"{b.locationNotes}"</p>}
                                                </div>

                                                <div className="flex gap-2">
                                                    <button onClick={() => handleBookingAction(b.id, 'Accepted')} className="flex-1 bg-green-600 text-white font-bold py-2 rounded-lg text-xs hover:bg-green-700 transition-colors shadow-sm">
                                                        ទទួល (Accept)
                                                    </button>
                                                    <button onClick={() => handleBookingAction(b.id, 'Rejected')} className="flex-1 bg-white border border-gray-200 text-red-600 font-bold py-2 rounded-lg text-xs hover:bg-red-50 transition-colors">
                                                        បដិសេធ (Reject)
                                                    </button>
                                                </div>
                                            </div>
                            ...
                            <div>
                                <h3 className="font-bold text-gray-800 mb-4 flex items-center">
                                    <CheckCircle className="h-5 w-5 mr-2 text-green-600" /> ថ្នាក់រៀន (Active & History)
                                </h3>
                                ...
                                <div className="space-y-3">
                                    {[...activeBookings, ...pastBookings].map(b => (
                                        <div key={b.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex flex-col md:flex-row items-center gap-4">
                                            <div className="flex items-center gap-4 flex-1 w-full">
                                                <div className={`w-2 h-12 rounded-full ${b.status === 'Accepted' ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                                                <img src={b.studentAvatar || `https://ui-avatars.com/api/?name=${b.studentName}`} className="w-10 h-10 rounded-full bg-gray-200" />
                                                <div>
                                                    <h4 className="font-bold text-gray-900">{b.studentName}</h4>
                                                    <p className="text-xs text-gray-500">{b.subject} • {b.scheduledTime}</p>
                                                </div>
                                            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl w-full max-w-sm p-6 animate-scale-in shadow-2xl">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => setShowApplyModal(false)}><X className="h-5 w-5 text-gray-400" /></button>
                        </div>
                        ...
                        <div className="bg-blue-50 p-3 rounded-xl mb-4 border border-blue-100">
                            ...
                            <p className="text-xs text-blue-700 mt-1">{selectedRequest.subject} • {selectedRequest.grade}</p>
                        </div>

                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">សារខ្លីៗ (Message)</label>
                                <textarea 
                            ...
                            >
                                {applying ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Send className="h-4 w-4 mr-2" /> ផ្ញើសារ</>}
                            </button>
                        </div>
                    </div>
                </div>
