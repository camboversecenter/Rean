...
import { Target, BookOpen, Filter, CheckCircle, Zap, X, Brain, Loader2, ChevronDown } from '../components/Icons';
...
const ExplorePage: React.FC = () => {
  ...
  const [formatFilter, setFormatFilter] = useState<'All' | 'Online' | 'On-Campus' | 'Hybrid'>('All');
      ...
      <div className="max-w-5xl mx-auto">
          ...
          <div className="flex overflow-x-auto space-x-2 mb-4 pb-2 scrollbar-hide">
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold transition-all ${
                    selectedCategory === 'All' ? 'bg-gray-800 text-white' : 'bg-white border border-gray-200 text-gray-600'
                }`}
                >
                ទាំងអស់
                </button>
                ...
                <button
                    ...
                    className={`whitespace-nowrap px-4 py-2 rounded-full text-xs font-bold transition-all ${
                    selectedCategory === cat ? 'bg-gray-800 text-white' : 'bg-white border border-gray-200 text-gray-600'
                    }`}
                >
                    {cat}
                </button>
              ...
              <div className="bg-gray-900 text-white rounded-2xl p-4 shadow-2xl flex items-center justify-between border border-gray-700">
                  <div className="flex items-center gap-3">
                      <div className="flex -space-x-2">
                        {compareList.map(c => (
                            <img key={c.id} src={c.coverImage || 'https://via.placeholder.com/50'} className="w-8 h-8 rounded-full border border-gray-800 object-cover" alt="cover"/>
                        ))}
                        {compareList.length < 2 && (
                            <div className="w-8 h-8 rounded-full border border-gray-800 bg-gray-800 flex items-center justify-center text-xs text-gray-500">?</div>
                        )}
                      </div>
                  ...
                  <button onClick={() => setCompareList([])} className="absolute -top-2 -right-2 bg-gray-700 rounded-full p-1 text-gray-300">
                      <X className="h-3 w-3" />
                  </button>
              </div>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
              <div className="bg-white rounded-2xl w-full max-w-lg max-h-[85vh] flex flex-col shadow-2xl animate-scale-in">
                  <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
                      ...
                      <button onClick={() => setShowCompareModal(false)}><X className="h-5 w-5 text-gray-400" /></button>
                  </div>
                  ...
                  <div className="p-6 overflow-y-auto flex-1">
                      {analyzing ? (
                          <div className="flex flex-col items-center justify-center py-10 text-center">
                              ...
                              <p className="text-sm text-gray-500">សុភាទន្សាយកំពុងអានព័ត៌មានលម្អិតជំនួសអ្នក...</p>
                          </div>
                      ) : (
                              ...
                              <div className="flex gap-4 border-b border-gray-100 pb-6">
                                  {compareList.map(c => (
                                      <div key={c.id} className="flex-1">
                                          <img src={c.coverImage || 'https://via.placeholder.com/150'} className="w-full h-24 object-cover rounded-lg mb-2 bg-gray-100" />
                  ...
                  <div className="p-4 border-t border-gray-100 bg-gray-50 rounded-b-2xl">
                      <button onClick={() => setShowCompareModal(false)} className="w-full bg-gray-900 text-white font-bold py-3 rounded-xl">
                          បិទ (Close)
                      </button>
                  </div>
              </div>
