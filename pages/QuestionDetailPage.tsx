...
import { fetchPostById, fetchReplies, createCommunityReply, toggleReaction, acceptAnswer, updateCommunityReply } from '../services/communityService';
...
import { 
    ChevronLeft, Loader2, Send, MessageCircle, CheckCircle, Ghost, 
    MoreHorizontal, User, Bookmark, Edit, Zap
...
const QuestionDetailPage: React.FC = () => {
    ...
    const loadReplies = async (pageToLoad: number, reset: boolean = false) => {
        if (!id) return;
    ...
    const handleSubmitReply = async () => {
        if (!replyContent.trim() || !post || !currentUser || replyContent.length > REPLY_LIMIT) {
            if (!currentUser) toast.error("Please login to reply.");
            return;
        }

        // --- AI TAGGING LOGIC ---
        const isTaggingAI = /@(tonsay|sopheatonsay)\b/i.test(replyContent);
        ...
        try {
            // 1. Save user reply
            const newReply = await createCommunityReply(post.id, replyContent, false);
            ...
            if (isTaggingAI) {
                ...
                const systemPrompt = `
                    ...
                    User's Reply (Tagging you): "${replyContent}"
                ...
                try {
                    ...
                    const aiResponseText = await chatWithAiTag(replyContent, systemPrompt);
    ...
    const handleSaveEdit = async (replyId: string) => {
        if (!editContent.trim() || editContent.length > REPLY_LIMIT) return;
        try {
            await updateCommunityReply(replyId, editContent);
        ...
        <div className="bg-gray-50 min-h-screen pb-32 font-sans">
            ...
            <div className="max-w-3xl mx-auto px-4 py-6">
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 mb-6">
                    <div className="flex items-start gap-4 mb-4">
                        {isPostAnonymous ? (
                            ...
                            <img src={post.authorAvatar || `https://ui-avatars.com/api/?name=${post.authorName}`} className="w-12 h-12 rounded-full object-cover border border-gray-200" />
                        )}
                        <div>
                            <h2 className={`font-bold text-gray-900 ${isPostAnonymous ? 'italic text-gray-500' : ''}`}>{displayAuthor}</h2>
                    ...
                    <div className="flex items-center gap-4 pt-4 border-t border-gray-50">
                        ...
                        <button onClick={() => replyInputRef.current?.focus()} className="flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-gray-700">
                ...
                <div className="space-y-4">
                    <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider ml-1">ចម្លើយទាំងអស់ ({replies.length})</h3>
                    ...
                    {replies.map(reply => (
                        <div key={reply.id} className={`bg-white rounded-xl p-4 shadow-sm border ${
                            reply.accepted ? 'border-green-500 ring-1 ring-green-500 bg-green-50/10' : 
                            reply.isAI ? 'border-blue-100 bg-blue-50/20' : 
                            'border-gray-100'
                        }`}>
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center gap-2">
                                    {reply.isAI ? (
                                        <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center"><span className="text-lg">🐰</span></div>
                                    ) : (
                                        <img src={reply.authorAvatar || `https://ui-avatars.com/api/?name=${reply.authorName}`} className="w-8 h-8 rounded-full object-cover" />
                                    )}
                                    <div>
                                        <p className={`text-sm font-bold ${reply.isAI ? 'text-primary' : 'text-gray-900'}`}>
                                            {reply.isAI && reply.authorName === 'Kru REAN' ? 'សុភាទន្សាយ (AI)' : reply.authorName}
                                        </p>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {!reply.accepted && !reply.isAI && currentUser?.id === post.author_id && (
                                        <button onClick={() => handleAcceptAnswer(reply)} className="text-gray-400 hover:text-green-600 p-1" title="Accept Answer">
                                            <CheckCircle className="h-5 w-5" />
                                        </button>
                                    )}
                                    {!reply.isAI && currentUser?.id === reply.author_id && editingReplyId !== reply.id && (
                                        <button onClick={() => handleEditClick(reply)} className="text-gray-400 hover:text-blue-600 p-1" title="Edit Reply">
                            ...
                            {editingReplyId === reply.id ? (
                                <div className="pl-10 mt-2 animate-fade-in">
                                    <textarea
                                        ...
                                        rows={3}
                                    />
                                    <CharCounter current={editContent.length} limit={REPLY_LIMIT} />
                                    <div className="flex justify-end gap-2 mt-2">
                                        <button onClick={handleCancelEdit} className="px-3 py-1.5 text-xs font-bold text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">បោះបង់ (Cancel)</button>
                                        <button 
                                            onClick={() => handleSaveEdit(reply.id)} 
                                            disabled={editContent.length > REPLY_LIMIT || !editContent.trim()}
                    ...
                    {hasMore && (
                        ...
                        >
                            {loadingMore ? <Loader2 className="h-4 w-4 animate-spin" /> : 'បង្ហាញបន្ថែម (Load More)'}
                        </button>
                    )}
                </div>
            </div>
                ...
                <div className="max-w-3xl mx-auto flex flex-col gap-1">
                    <div className="flex gap-3">
                        ...
                        <button 
                            ...
                            disabled={isReplying || !replyContent.trim() || replyContent.length > REPLY_LIMIT}
                        ...
                        >
                            {isReplying ? <Loader2 className="h-5 w-5 animate-spin" /> : (
                                isTaggingAI ? (
                    ...
                    <div className="flex justify-between items-center pr-2 pl-2 mt-1">
                        <p className="text-[10px] text-gray-400">
                            Tip: Tag <span className="font-bold text-primary cursor-pointer hover:underline" onClick={() => setReplyContent(prev => prev + '@tonsay ')}>@tonsay</span> to get AI help ({AI_COSTS.TAG_REPLY} pts).
                        </p>
                        <CharCounter current={replyContent.length} limit={REPLY_LIMIT} className="mt-0" />
                    </div>
                </div>
            </div>
        </div>
