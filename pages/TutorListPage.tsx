...
import { fetchAllTutors, fetchStudentRequests, createStudentRequest, deleteStudentRequest, applyToRequest } from '../services/tutorService';
import { getCurrentUserProfile } from '../services/authService';
import { TutorProfile, TutorRequest } from '../types';
import { Loader2, Plus, X, Clock, RefreshCw, Filter, Search, Send, ChevronDown } from '../components/Icons';
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex justify-between items-start mb-4">
            <div>
                ...
                <p className="text-xs text-gray-500 mt-1">ស្វែងរកគ្រូ ឬប្រកាសពីតម្រូវការសិក្សារបស់អ្នក</p>
            </div>
            <button 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                 {tutors.length === 0 && (
                     <div className="col-span-full flex flex-col items-center justify-center py-16 text-center bg-white rounded-2xl border border-dashed border-gray-200">
                         ...
                         <p className="text-gray-500 text-xs">សូមព្យាយាមប្តូរប្រភេទស្វែងរក ឬត្រឡប់មកវិញពេលក្រោយ។</p>
                     </div>
                 )}
                 {tutors.map(tutor => (
               ...
               <div className="space-y-4">
                  ...
                  {!isTutor && (
                      <div className="bg-gradient-to-r from-primary/10 to-teal-100/50 border border-primary/10 rounded-2xl p-5 flex items-center justify-between relative overflow-hidden">
                          <div className="relative z-10">
                             ...
                             <p className="text-xs text-gray-600">ប្រកាសពីតម្រូវការរបស់អ្នក ដើម្បីឱ្យគ្រូទាក់ទងមក។</p>
                          </div>
                          <button 
                  ...
                  <div className="flex items-center justify-between mt-6 mb-2">
                      <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wider">សំណើថ្មីៗ (Latest Requests)</h3>
                  </div>
      ...
      {showApplyModal && selectedRequest && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
              <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-scale-in shadow-2xl">
                  ...
                  <div className="space-y-4">
                      <div>
                          <label className="block text-xs font-bold text-gray-500 mb-1">សារខ្លីៗទៅកាន់សិស្ស</label>
                          <textarea 
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
              <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-scale-in shadow-2xl">
                  <div className="flex justify-between items-center mb-6">
                      ...
                      <button onClick={() => setShowRequestModal(false)} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><X className="h-5 w-5 text-gray-400" /></button>
                  </div>
                  <div className="space-y-4">
                      <div>
                          <label className="block text-xs font-bold text-gray-500 mb-1">មុខវិជ្ជា</label>
                          <input placeholder="ឧ. គណិតវិទ្យា, រូបវិទ្យា..." className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-primary/20 outline-none" onChange={e => setNewRequest({...newRequest, subject: e.target.value})} />
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                          <div>
                              <label className="block text-xs font-bold text-gray-500 mb-1">កម្រិត</label>
                              <input placeholder="ឧ. ថ្នាក់ទី១២" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-primary/20 outline-none" onChange={e => setNewRequest({...newRequest, grade: e.target.value})} />
                          </div>
                          <div>
                              <label className="block text-xs font-bold text-gray-500 mb-1">ថវិកា (រៀល)</label>
                              <input type="number" placeholder="ឧ. 20000" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-primary/20 outline-none" onChange={e => setNewRequest({...newRequest, budget: e.target.value})} />
                          </div>
                      </div>
                      <div>
                          <label className="block text-xs font-bold text-gray-500 mb-1">ទីតាំង</label>
                          <input placeholder="ទីតាំង ឬ អនឡាញ" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:ring-2 focus:ring-primary/20 outline-none" onChange={e => setNewRequest({...newRequest, location: e.target.value})} />
                      </div>
                      <div>
                          <label className="block text-xs font-bold text-gray-500 mb-1">ព័ត៌មានលម្អិត</label>
                          <textarea placeholder="ពណ៌នាអំពីតម្រូវការរបស់អ្នក..." className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm h-24 focus:bg-white focus:ring-2 focus:ring-primary/20 outline-none resize-none" onChange={e => setNewRequest({...newRequest, description: e.target.value})} />
                      </div>
                      <button 
