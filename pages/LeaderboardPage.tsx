            ...
            <div className="max-w-5xl mx-auto">
                ...
                {users.length > 0 && (
                    <div className="mb-12 relative flex justify-center items-end gap-4 md:gap-8 h-64 md:h-80 animate-fade-in">
                        ...
                        {/* 2nd Place */}
                        {users[1] && (
                            <Link to={`/profile/${users[1].id}`} className="flex flex-col items-center group w-1/3 max-w-[140px]">
                                <div className="relative mb-2 transition-transform group-hover:-translate-y-2">
                                    <img 
                                        src={users[1].avatar_url || `https://ui-avatars.com/api/?name=${users[1].full_name}`} 
                                        className="w-16 h-16 md:w-20 md:h-20 rounded-full border-4 border-gray-300 shadow-lg object-cover"
                                        alt="2nd"
                                    />
                                    <div className="absolute -bottom-3 inset-x-0 flex justify-center">
                                        <span className="bg-gray-300 text-white text-xs md:text-sm font-bold px-2.5 py-0.5 rounded-full shadow-sm ring-2 ring-white">2nd</span>
                                    </div>
                                </div>
                                <div className="text-center mb-2 mt-2">
                                    <p className="font-bold text-gray-900 text-sm md:text-base line-clamp-1">{users[1].full_name}</p>
                        ...
                        {/* 1st Place */}
                        {users[0] && (
                            <Link to={`/profile/${users[0].id}`} className="flex flex-col items-center z-10 w-1/3 max-w-[160px] group">
                                <div className="relative mb-2 transition-transform group-hover:-translate-y-2">
                                    <div className="absolute -top-8 inset-x-0 flex justify-center animate-bounce">
                                        <span className="text-4xl filter drop-shadow-md">👑</span>
                                    </div>
                                    <img 
                                        src={users[0].avatar_url || `https://ui-avatars.com/api/?name=${users[0].full_name}`} 
                                    ...
                                    />
                                    <div className="absolute -bottom-3 inset-x-0 flex justify-center">
                                        <span className="bg-yellow-400 text-white text-sm md:text-base font-bold px-3 py-0.5 rounded-full shadow-sm ring-2 ring-white">1st</span>
                                    </div>
                                </div>
                                <div className="text-center mb-2 mt-4">
                                    <p className="font-bold text-gray-900 text-base md:text-lg line-clamp-1">{users[0].full_name}</p>
                                    <p className="text-xs text-yellow-600 font-bold font-mono bg-yellow-50 px-2 py-0.5 rounded-full">{users[0].lifetime_xp} XP</p>
                                </div>
                        ...
                        {/* 3rd Place */}
                        {users[2] && (
                            <Link to={`/profile/${users[2].id}`} className="flex flex-col items-center group w-1/3 max-w-[140px]">
                                <div className="relative mb-2 transition-transform group-hover:-translate-y-2">
                                    <img 
                                        src={users[2].avatar_url || `https://ui-avatars.com/api/?name=${users[2].full_name}`} 
                                        className="w-16 h-16 md:w-20 md:h-20 rounded-full border-4 border-orange-300 shadow-md object-cover"
                                        alt="3rd"
                                    />
                                    <div className="absolute -bottom-3 inset-x-0 flex justify-center">
                                        <span className="bg-orange-300 text-white text-xs md:text-sm font-bold px-2.5 py-0.5 rounded-full shadow-sm ring-2 ring-white">3rd</span>
                                    </div>
                                </div>
                                <div className="text-center mb-2 mt-2">
                                    <p className="font-bold text-gray-900 text-sm md:text-base line-clamp-1">{users[2].full_name}</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 animate-fade-in delay-100">
                        {restUsers.map((user, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <div className="w-8 md:w-10 font-bold text-gray-400 text-center mr-2 md:mr-4 font-mono text-lg">#{rank}</div>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <h4 className="font-bold text-gray-900 text-sm md:text-base truncate group-hover:text-primary transition-colors">{user.full_name}</h4>
                                        <p className="text-xs text-gray-500 truncate capitalize">{user.role || 'Student'}</p>
                                    </div>
                                    <div className="text-right">
                                        <span className="font-bold text-gray-900 text-sm md:text-base block">{user.lifetime_xp} XP</span>
                                        <span className="text-[10px] text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full inline-block mt-1">Lvl {Math.floor((user.lifetime_xp || 0) / 100) + 1}</span>
                                    </div>
                                </Link>
                            );
                        })}
