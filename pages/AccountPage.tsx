...
import { User, Camera, LogOut, Save, Loader2, AlertCircle, Building2, ChevronRight, Target, PlayCircle, Zap, GraduationCap, Clock, Award, Gift, CheckCircle, X, Bell, FileText, Monitor, BookOpen, HelpCircle, Edit } from './Icons';
...
const AccountPage: React.FC = () => {
  ...
  const incomingApplications = bookings.filter(b => 
      b.status === 'Pending' && b.locationNotes?.includes('[Job Application]')
  );

  const myPendingRequests = bookings.filter(b => 
      b.status === 'Pending' && !b.locationNotes?.includes('[Job Application]')
  );

  const activeBookings = bookings.filter(b => b.status === 'Accepted' || b.status === 'Completed' || b.status === 'Rejected');
    ...
    <div className="min-h-screen bg-gray-50 pb-24 font-sans animate-fade-in">
      ...
      <div className="bg-white border-b border-gray-200 mb-8 relative">
        ...
        <div className="max-w-5xl mx-auto px-4 pt-6 pb-2 relative z-20">
            <h1 className="text-xl font-bold text-gray-900">គណនីរបស់ខ្ញុំ (My Account)</h1>
        </div>
            ...
            <div className="flex flex-col md:flex-row items-center md:items-end -mt-12 md:-mt-16 gap-4 md:gap-6">
                ...
                <div className="relative group flex-shrink-0">
                    <div className="w-24 h-24 md:w-36 md:h-36 rounded-full border-4 border-white shadow-lg overflow-hidden bg-gray-100">
                        <img 
                            src={previewUrl || profile?.avatar_url || `https://ui-avatars.com/api/?name=${fullName}`} 
                            alt="Profile" 
                ...
                {/* User Info - Adjusted margin for desktop */}
                <div className="flex-1 text-center md:text-left mb-2 md:mb-1">
                    <h1 className="text-2xl md:text-3xl font-bold text-gray-900 leading-tight mb-1">{profile?.full_name}</h1>
                ...
                {levelStats && (
                    <div className="w-full md:w-80 bg-white md:bg-gray-50 p-4 rounded-2xl border border-gray-100 md:border-gray-200 shadow-sm md:shadow-none mb-2 md:mb-4">
                        <div className="flex justify-between items-end mb-2">
                            <div>
                                ...
                                <div className="text-lg font-black text-gray-900 leading-none mt-0.5">
                                    {profile?.lifetime_xp} <span className="text-xs font-bold text-gray-400">XP</span>
                                </div>
                            </div>
                            <div className="text-right">
                                <div className="text-lg font-black text-primary leading-none">
                                    {profile?.spendable_points} <span className="text-xs font-bold text-gray-400">Pts</span>
                                </div>
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden mb-4">
                            <div className="bg-gradient-to-r from-secondary to-primary h-full rounded-full transition-all duration-500" style={{ width: `${levelStats.percent}%` }}></div>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                            <Link to="/leaderboard" className="flex items-center justify-center py-2 bg-white rounded-lg border border-gray-200 text-xs font-bold text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all">
                                <Award className="h-4 w-4 mr-1.5 text-yellow-500" /> Leaderboard
                            </Link>
                            <Link to="/rewards" className="flex items-center justify-center py-2 bg-white rounded-lg border border-gray-200 text-xs font-bold text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all">
             ...
             <div className="md:col-span-2 space-y-8">
                 ...
                 {(isTutor || isSchool || isCreator) && (
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {isTutor && (
                            <Link to="/tutor/dashboard" className="bg-white border border-green-100 p-5 rounded-2xl shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
                        ...
                        {isCreator && (
                            <Link to="/creator" className="bg-white border border-indigo-100 p-5 rounded-2xl shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
                        ...
                        {isSchool && (
                            <Link to="/school/dashboard" className="bg-white border border-blue-100 p-5 rounded-2xl shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
                 ...
                 {incomingApplications.length > 0 && (
                     <section>
                        <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center">
                            <Bell className="h-5 w-5 mr-2 text-yellow-500 fill-yellow-500" /> សំណើពីគ្រូ (Job Applications)
                        </h2>
                        <div className="space-y-4">
                            {incomingApplications.map((booking) => (
                                <div key={booking.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:border-primary/30 transition-colors">
                                    <div className="flex flex-col sm:flex-row justify-between gap-4">
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <h4 className="font-bold text-gray-900">{booking.tutorName || 'គ្រូបង្រៀន'}</h4>
                                                <span className="bg-yellow-100 text-yellow-800 text-[10px] font-bold px-2 py-0.5 rounded">Applying</span>
                                            </div>
                                            <p className="text-sm text-gray-600">Applied for: <span className="font-bold">{booking.subject}</span></p>
                     ...
                     <section>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {myPendingRequests.map((booking) => (
                                <div key={booking.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 opacity-75 hover:opacity-100 transition-opacity">
                                    <div className="flex justify-between items-start mb-2">
                                        <h4 className="font-bold text-gray-900 text-sm">{booking.tutorName || 'គ្រូបង្រៀន'}</h4>
                                        <span className="text-[10px] font-bold px-2 py-1 rounded bg-gray-100 text-gray-500">Waiting</span>
                                    </div>
                                    <div className="text-xs text-gray-500 mb-1">
                                        <span className="font-medium">{booking.subject}</span>
                                    </div>
                                    <p className="text-xs text-gray-400 italic">Sent on {new Date(booking.createdAt).toLocaleDateString()}</p>
                                </div>
                            ))}
                        </div>
                     </section>
                 ...
                 {activeBookings.length > 0 && (
                     <section>
                        <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center">
                            <GraduationCap className="h-5 w-5 mr-2 text-primary" /> ថ្នាក់រៀនរបស់ខ្ញុំ (My Classes)
                        </h2>
                        <div className="space-y-3">
                            {activeBookings.map((booking) => (
                                <div key={booking.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center justify-between hover:shadow-md transition-shadow">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-2 h-10 rounded-full ${booking.status === 'Accepted' ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                                        <div>
                                            <h4 className="font-bold text-gray-900 text-sm">{booking.tutorName}</h4>
                                            <p className="text-xs text-gray-500">{booking.subject} • {booking.status}</p>
                                        </div>
                                    </div>
                                    {booking.status === 'Accepted' && (
                                        <Link to={`/classroom/${booking.id}`} className="text-xs bg-gray-900 text-white px-4 py-2 rounded-xl font-bold hover:bg-black transition-colors flex items-center">
                    ...
                    <section>
                        ...
                        <div className="space-y-3">
                            {activeMissions.map((m) => (
                                <div key={m.enrollmentId} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center justify-between hover:border-primary/30 transition-colors">
                                    <div className="flex items-center gap-4 overflow-hidden">
                                        <img src={m.thumbnail || 'https://via.placeholder.com/80'} alt={m.title} className="w-12 h-12 rounded-xl object-cover flex-shrink-0 bg-gray-100" />
                                        <div className="min-w-0">
                                            <h4 className="font-bold text-gray-900 text-sm truncate">{m.title}</h4>
                                            <p className="text-xs text-gray-500 flex items-center mt-0.5">
                                                <span className="w-2 h-2 rounded-full bg-green-500 mr-1.5"></span> In Progress
                                            </p>
                                        </div>
                                    </div>
                                    <Link to={`/mission/${m.id}`} className="bg-primary/5 text-primary p-2.5 rounded-full hover:bg-primary/10 flex-shrink-0 transition-colors">
                    ...
                    {achievements.length === 0 ? (
                        ...
                        <div className="space-y-3">
                            {achievements.map((item) => (
                                <div key={item.id} className="bg-gradient-to-r from-yellow-50 to-white rounded-xl border border-yellow-100 p-3 flex items-center gap-3">
