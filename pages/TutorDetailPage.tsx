...
import { Loader2, MapPin, CheckCircle, GraduationCap, Monitor, Phone, Calendar, Clock, X, ChevronLeft, Share2 } from '../components/Icons';
        ...
        <div className="bg-gray-50 min-h-screen pb-32 font-sans">
            ...
            <div className="relative pt-12 pb-16 px-4 text-white shadow-lg overflow-hidden">
                ...
                <div className="absolute top-4 left-4 right-4 flex justify-between items-center z-10">
                    <button onClick={() => navigate(-1)} className="p-2 bg-white/20 rounded-full hover:bg-white/30 backdrop-blur-md transition-colors">
                ...
                <div className="max-w-5xl mx-auto flex flex-col items-center text-center relative z-10">
                    <div className="w-24 h-24 rounded-full border-4 border-white shadow-lg overflow-hidden mb-4 bg-gray-200">
                        <img src={tutor.avatarUrl || `https://ui-avatars.com/api/?name=${tutor.fullName}`} className="w-full h-full object-cover" />
                    </div>
                    <h1 className="text-2xl font-bold flex items-center gap-2">
                        {tutor.fullName} {tutor.isVerified && <CheckCircle className="h-5 w-5 text-blue-300 fill-blue-500" />}
                    </h1>
                    ...
                    <div className="flex gap-2 mt-4">
                        ...
                        <span className="bg-white/20 px-3 py-1 rounded-full text-xs backdrop-blur-sm flex items-center border border-white/10">
                            {tutor.teachingMode === 'Online' ? <Monitor className="h-3 w-3 mr-1" /> : <GraduationCap className="h-3 w-3 mr-1" />}
                            {tutor.teachingMode === 'Online' ? 'អនឡាញ' : (tutor.teachingMode === 'Home' ? 'រៀននៅផ្ទះ' : 'ទាំងពីរ')}
                        </span>
                    </div>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-4">
                        <div className="bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
                            <div className="flex justify-between items-center mb-4">
                                <div>
                                    ...
                                    <p className="text-xl font-bold text-primary">{formatRiel(tutor.hourlyRate || 0)} <span className="text-xs text-gray-400 font-normal">/h</span></p>
                                </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
                            ...
                            <div className="flex flex-wrap gap-2 mb-4">
                                {tutor.subjects?.map(s => <span key={s} className="bg-blue-50 text-blue-700 px-3 py-1 rounded-lg text-sm font-medium">{s}</span>)}
                            </div>
                            <div className="flex flex-wrap gap-2">
                                {tutor.grades?.map(g => <span key={g} className="bg-gray-100 text-gray-600 px-3 py-1 rounded-lg text-xs">{g}</span>)}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-4">
                        <div className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 min-h-[300px]">
                            <h3 className="font-bold text-gray-900 mb-3 text-lg border-b border-gray-100 pb-2">អំពីខ្ញុំ (About Me)</h3>
                            <p className="text-gray-600 text-sm leading-relaxed whitespace-pre-wrap">{tutor.bio || "មិនមានការពិពណ៌នា"}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl w-full max-w-sm p-6 animate-scale-in shadow-2xl">
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => setShowBooking(false)} className="p-1 hover:bg-gray-100 rounded-full"><X className="h-6 w-6 text-gray-400" /></button>
                        </div>
                        
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">មុខវិជ្ជា</label>
                                <select className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary/20" onChange={e => setBookingForm({...bookingForm, subject: e.target.value})}>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="date" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary/20" onChange={e => setBookingForm({...bookingForm, date: e.target.value})} />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 mb-1">ម៉ោង</label>
                                    <input type="time" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-primary/20" onChange={e => setBookingForm({...bookingForm, time: e.target.value})} />
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1">ចំណាំ (Notes)</label>
                                <textarea className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm h-24 outline-none focus:ring-2 focus:ring-primary/20 resize-none" placeholder="តើអ្នកចង់រៀនអំពីអ្វី?" onChange={e => setBookingForm({...bookingForm, notes: e.target.value})} />
                            </div>
