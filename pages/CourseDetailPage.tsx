...
import { 
    Calendar, Clock, MapPin, Monitor, CheckCircle, 
    User, BookOpen, AlertCircle, X, Loader2, Building2,
...
const CourseDetailPage: React.FC = () => {
    ...
    const [course, setCourse] = useState<ShortCourse & { school: { id: string, name: string, logo: string, description?: string, location?: string } } | null>(null);
        ...
        const load = async () => {
            ...
            if (data) {
                ...
                const related = allCourses.filter(c => 
                    c.id !== data.id && 
                    (c.category === data.category || c.schoolId === data.school.id)
                ).slice(0, 3);
             ...
             <div className="relative h-64 md:h-80 bg-gray-900">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-gray-900 via-gray-900/60 to-transparent">
                     <div className="max-w-5xl mx-auto">
                        ...
                        <Link to={`/school/${course.school.id}`} className="flex items-center text-gray-200 hover:text-white transition-colors w-fit">
                ...
                <div className="bg-white rounded-xl shadow-lg shadow-gray-200/50 border border-gray-100 p-5">
                    <div className="flex flex-wrap gap-y-3 gap-x-6 text-sm text-gray-600 mb-5">
                        <div className="flex items-center">
                            ...
                            <span>ចាប់ផ្តើម៖ <span className="font-bold text-gray-900">{course.startDate}</span></span>
                        </div>
                        {course.deadline && (
                            <div className="flex items-center">
                                <AlertCircle className={`h-4 w-4 mr-2 ${isExpired ? 'text-red-500' : 'text-primary'}`} />
                        ...
                        <div className="flex items-center">
                            {course.format === 'Online' ? <Monitor className="h-4 w-4 mr-2 text-primary" /> : <MapPin className="h-4 w-4 mr-2 text-primary" />}
                            <span>{course.format === 'Online' ? 'អនឡាញ' : (course.format === 'On-Campus' ? 'រៀននៅសាលា' : 'ចម្រុះ')}</span>
                        </div>
                    ...
                    <div className="flex items-center justify-between border-t border-gray-100 pt-4">
                        <div className="flex flex-col">
                            <span className="text-xs text-gray-500 uppercase font-bold tracking-wider mb-0.5">តម្លៃសិក្សា (Tuition Fee)</span>
                        ...
                        <div className="space-y-4 animate-fade-in">
                            {course.syllabus && course.syllabus.length > 0 ? (
                                course.syllabus.map((topic, idx) => (
                                    <div key={idx} className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm group hover:border-primary/30 transition-colors">
                        ...
                        <div className="space-y-6 animate-fade-in">
                             <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm">
                                <div className="flex items-center mb-6">
                                    <img src={course.school.logo} className="w-16 h-16 rounded-xl border border-gray-100 mr-4 p-1" alt="School Logo" />
                                    <div>
                                        <h3 className="font-bold text-gray-900 text-lg">{course.school.name}</h3>
                                        <Link to={`/school/${course.school.id}`} className="text-sm text-primary hover:underline font-medium">មើលប្រវត្តិពេញ (Full Profile)</Link>
                                    </div>
                                </div>
                                ...
                                <p className="text-sm text-gray-600 leading-relaxed mb-6">
                                    ...
                                    វគ្គសិក្សានេះផ្តល់ជូនដោយ {course.school.name} ដែលជាគ្រឹះស្ថានអប់រំឈានមុខគេមួយក្នុងវេទិកា REAN។
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden animate-scale-in shadow-2xl">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase">ឈ្មោះរបស់អ្នក</label>
                                <input 
                                    type="text" 
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 mb-1.5 uppercase">លេខទូរស័ព្ទ <span className="text-red-500">*</span></label>
                                <input 
                                    type="tel" 
